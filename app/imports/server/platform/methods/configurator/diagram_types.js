...
Meteor.methods({
	...
	insertDiagramType: async function(list) {
		...
		if (await is_system_admin(user_id) && list) {
			...
			await DialogTabs.insertAsync({toolId: list["toolId"],
	...
	reorderTabIndexes: async function(list) {
		...
		if (await is_system_admin(user_id, list)) {
        	...
        	if (prev_index < current_index) {
	       		await DialogTabs.updateAsync({$and: [{index: {$gt: prev_index}},
    										{_id: {$ne: tab_id}}, query]},
    								{$inc: {index: current_index}}, {multi: true});
        	}
        	else {
        		await DialogTabs.updateAsync({$and: [query,
	...
	reorderPaletteButtonIndexes: async function(list) {
		...
		if (await is_system_admin(user_id, list)) {
        	...
        	if (prev_index < current_index) {
	       		...
	       		await PaletteButtons.updateAsync({$and: [{index: {$gt: prev_index}},
    										{_id: {$ne: button_id}}, query]},
    								{$inc: {index: current_index}}, {multi: true});
        	}
        	else {

        		await PaletteButtons.updateAsync({$and: [query,
    										{$or: [{index: {$gt: prev_index}},
    												{_id: button_id}]}
    										]},
    								{$inc: {index: prev_index}},
