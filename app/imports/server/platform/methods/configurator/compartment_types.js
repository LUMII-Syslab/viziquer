...
import { CompartmentTypes, Compartments, Elements, ElementTypes, DialogTabs } from '/imports/db/platform/collections'
...
Meteor.methods({
  ...
  reorderCompartmentTypeTabIndexes: async function (list) {
    ...
    if (is_system_admin(user_id, list)) {
      ...
      //if (prev_index > 1000) {
      //	var compart_type = CompartmentTypes.findOneAsync({toolId: tool_id, dialogTabId: tab_id},
      //							{sort: {tabIndex: 1}});
      //	if (compart_type) {
