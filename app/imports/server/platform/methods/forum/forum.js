...
import { ForumPosts, ForumPostComments, ForumPostTags } from '/imports/db/platform/collections'
...
Meteor.methods({
  ...
  editPost: function (list) {
    ...
    if (is_project_member(user_id, list)) {
      ...
      ForumPosts.update({
        _id: list["postId"], projectId: list["projectId"],
        authorId: user_id
      },
        {
          $set: {
...
async function add_forum_post_tag(post_id, proj_id, tag) {
  ...
  var tmp = await ForumPostTags.findOneAsync({
    projectId: proj_id,
    name: tag,
    forumPosts: post_id
  });
