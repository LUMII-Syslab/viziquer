...
import { ProjectsUsers, ProjectsGroups, Versions, UserVersionSettings, Searches, Users, Projects, Diagrams, Elements, Compartments, Tools, DiagramTypes, ElementTypes, CompartmentTypes, PaletteButtons, DialogTabs, DiagramLogs } from '/imports/db/platform/collections'
...
Meteor.publish("Diagrams", async function (list) {
  ...
  if (is_project_version_reader(user_id, list, role)) {
    ...
    var fields = {
      _id: 1, name: 1, imageUrl: 1, seenCount: 1, createdAt: 1, isLayoutComputationNeededOnLoad: 1,
...
Meteor.publish("FoundDiagrams", async function (list) {
  ...
  if (is_project_version_reader(user_id, list, role)) {
    ...
    if (list["text"] && list["text"] != "") {
      ...
      var comparts = Compartments.find({
        projectId: proj_id, versionId: version_id,
...
Meteor.publish("DiagramTypes_UserVersionSettings", async function (list) {
  ...
  if (is_project_version_reader(user_id, list)) {
    ...
    if (version) {
      ...
      var diagram_type_fields = {
        fields: {
          _id: 1, name: 1, editorType: 1,
          extensionPoints: 1, style: 1, layoutSettings: 1,
        }
      };
      var user_version_settings = {
        fields: {
          userSystemId: 0,
          documentsSortBy: 0,
      ...
      return [DiagramTypes.find({
        toolId: version["toolId"],
        versionId: version["toolVersionId"]
      }, diagram_type_fields),
      UserVersionSettings.find({
        projectId: list["projectId"],
...
Meteor.publish("Diagram_Palette_ElementType", async function (list) {
  ...
  var diagram_limit = {
    fields: {
      _id: 1, name: 1, imageUrl: 1, style: 1, seenCount: 1, isPublic: 1,
  ...
  if (is_project_version_reader(user_id, list, role) || (await is_public_diagrams(diagrams))) {
    ...
    if (version) {
      ...
      var diagram_type_limit = {
        fields: {
          diagramId: 0, diagramTypeId: 0,
...
Meteor.publish("DiagramLogs", async function (list) {
  ...
  if (is_project_version_reader(user_id, list, role)) {
    ...
    return Meteor.publishWithRelations({
      ...
      filter: {
        diagramId: list["diagramId"], projectId: list["projectId"],
        versionId: list["versionId"]
      },
...
function limit_element_fields() {
  return {
    _id: 1, type: 1, style: 1, styleId: 1, location: 1, startElement: 1, endElement: 1, layoutSettings: 1,
