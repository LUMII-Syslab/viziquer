...
Meteor.methods({
  ...
  exportToolConfiguration: async function (list) {

    var export_tool = new ExportDiagramConfig();
    var res = await export_tool.export(list.toolId);
...
ExportDiagramConfig.prototype = {
  ...
  export: async function (tool_id) {

    await this.exportTool(tool_id);
