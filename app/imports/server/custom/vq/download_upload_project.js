    ...
    if (is_project_member(user_id, list) || is_public_diagram(list["diagramId"])) {
      ...
      var diagrams = await Diagrams.find({ projectId: project_id, versionId: version_id }).mapAsync(async function (diagram) {
        ...
        diagram.elements = await Elements.find({ diagramId: diagram._id, projectId: project_id, versionId: version_id }).mapAsync(async function (element) {
          ...
          element.compartments = await Compartments.find({
...
async function uploadProject(list) {
  ...
  if (is_project_member(user_id, list)) {
    ...
    await Projects.updateAsync({ _id: project_id, }, {
      $set: {
    ...
    await data.diagrams.forEachAsync(async function (diagram) {
      ...
      elements.forEachAsync(async function (element) {
        ...
        if (!element_type) {
          ...
          if (!element_type) {
            ...
            element_type = await ElementTypes.findOneAsync({
        ...
        if (element.type == "Line") {
          _.extend(element, {
            startElement: elem_map[element.startElement],
        ...
        compartments.forEachAsync(async function (compartment) {
          ...
          if (!compart_type) {
            ...
            if (!compart_type) {
              ...
              compart_type = await CompartmentTypes.findOneAsync({
                name: compart_type_name,
                toolId: tool_id,
                diagramTypeId: diagram_type._id,
                elementTypeId: element_type._id,
              });
