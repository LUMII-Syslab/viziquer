...
Meteor.publish("ViewData", async function (list) {
  ...
  if (await is_project_member(user_id, list)) {
    ...
    var view_filter = await ViewFilter.findOneAsync({
      viewId: list.viewId,
      projectId: list.projectId,
      versionId: list.versionId,
    });
...
async function collect_data(id, fields, list, view_filter, change) {
  ...
  if (change) {
    if (
      ...
      return {
        netChartData: { nodes: nodes, edges: edges },
      ...
      var time_chart_data = await collect_time_chart_data(
        data_links,
        actual_key_index,
      );
...
async function collect_time_chart_data(data_links, actual_key_index) {
  ...
  await data_links.forEachAsync(function (data_elem) {
    ...
    timeInterval = [
      Math.min(timeInterval[0], startS - 1),
      Math.max(timeInterval[1], endS + 1),
    ];
  ...
  return {
    data: { values: values, interval: timeInterval },
...
function build_box_query(fields, node_ids, list) {
  ...
  box_query = {
    $and: [{ projectId: list.projectId, versionId: list.versionId }, box_query],
  };
...
async function collect_transactions_table(list, fields) {
  ...
  if (
    fields.filter &&
    fields.filter.transactionsFilter &&
    fields.filter.transactionsFilter.transactions &&
    fields.filter.transactionsFilter.transactions.value
  ) {
    query["data.value"] = {
      $regex: fields.filter.transactionsFilter.transactions.value,
      $options: "i",
    };
  }

  if (fields.filter && fields.filter.timeChartFilter) {
    var time_filter = build_time_chart_query({
      filter: { timeChartFilter: fields.filter.timeChartFilter },
    });
  ...
  if (
    ...
    fields.filter.transactionsFilter &&
    fields.filter.transactionsFilter.transactions &&
    fields.filter.transactionsFilter.transactions.page
  ) {
  ...
  return {
    transactions: await data_links.fetchAsync(),
    transactionsCount: await data_links.countAsync(),
...
function build_time_chart_query(fields) {
  if (fields.filter && fields.filter.timeChartFilter) {
    ...
    if (time_chart_filter.edges) {
      ...
      return {
        startDate: { $gte: new Date(time_filters.startDate) },
