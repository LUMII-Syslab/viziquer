...
Meteor.publish("FoundDiagrams", async function (list) {
  ...
  if (await is_project_version_reader(user_id, list, role)) {
    ...
    if (list.text && list.text !== "") {
      ...
      var diagram_ids = await comparts.mapAsync(function (compart) {
        return compart.diagramId;
      });

      var diagram_regex = {
        $regex: make_regex_subsring(list.text),
        $options: "i",
      };
...
Meteor.publish("DiagramTypes_UserVersionSettings", async function (list) {
  ...
  if (await is_project_version_reader(user_id, list)) {
    ...
    if (version) {
      ...
      return [
        DiagramTypes.find(
          { toolId: version.toolId, versionId: version.toolVersionId },
          diagram_type_fields,
        ),
...
Meteor.publish("Diagram_Palette_ElementType", async function (list) {
  ...
  if (
    (await is_project_version_reader(user_id, list, role)) ||
    (await is_public_diagrams(diagrams))
  ) {
    var version = await Versions.findOneAsync({
      _id: list.versionId,
      projectId: list.projectId,
    ...
    if (version) {
      ...
      var diagram_type_limit = {
        fields: { diagramId: 0, diagramTypeId: 0, toolId: 0, versionId: 0 },
      };
...
Meteor.publish("Diagram_Types", async function (list) {
  ...
  if (
    (await is_project_version_reader(user_id, list, role)) ||
    (await is_public_diagrams(diagram))
  ) {
    var version = await Versions.findOneAsync({
      _id: list.versionId,
      projectId: list.projectId,
...
Meteor.publish("Diagram_Locker", async function (list) {
  ...
  if (
    (await is_project_version_reader(this.userId, list)) ||
    (await is_public_diagrams(diagram))
  ) {
    var self = this;
    ...
    var handle = diagram.observe({
      ...
      changed: async function (new_doc, old_doc) {
        var user = await Users.findOneAsync({
          systemId: new_doc.editingUserId,
        });
...
async function select_project_users(user_id, list) {
  ...
  if (list.projectId) query.projectId = list.projectId;

  var proj_ids = await ProjectsUsers.find(query).mapAsync(function (proj_user) {
