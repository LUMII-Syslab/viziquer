...
} from "./initialTypes/diagram_types.js";
import { error_msg } from "../../_global_functions.js";

DiagramTypes.after.update(
  async function (user_id, doc, fields, modifier, options) {
    if (!modifier || !modifier.$set) return false;
        ...
        await CompartmentTypes.updateAsync(
          ...
          async function (err, res) {
            if (err) {
              ...
              var extension_point = {
                extensionPoint: attr_name,
                procedure: attr_value,
              };

              await CompartmentTypes.updateAsync(query2, {
                $push: { extensionPoints: extension_point },
              });
            }
          },
        );
        ...
        await ElementTypes.updateAsync(
          ...
          async function (err, res) {
            if (err) {
              ...
              await ElementTypes.updateAsync(query2, {
                $push: { extensionPoints: extension_point },
              });
            }
          },
        ...
        await DiagramTypes.updateAsync(
          ...
          async function (err, res) {
            if (err) {
              ...
              await DiagramTypes.updateAsync(query2, {
                $push: { extensionPoints: extension_point },
              });
            }
          },
        ...
        await DialogTabs.updateAsync(
          {
            $and: [
              query,
              { $or: [{ index: { $gt: prev_index } }, { _id: tab_id }] },
            ],
          },
        ...
        await PaletteButtons.updateAsync(
          {
            $and: [
              query,
              { $or: [{ index: { $gt: prev_index } }, { _id: button_id }] },
            ],
          },
          { $inc: { index: prev_index } },
          { multi: true },
  ...
  updateDiagramTypeStyle: async function (list) {
    ...
    if (await is_system_admin(user_id, list)) {
      ...
      await Diagrams.updateAsync(
        { $or: [{ _id: list["diagramId"] }, { diagramTypeId: list["id"] }] },
        { $set: update },
        { multi: true },
      );
    }
