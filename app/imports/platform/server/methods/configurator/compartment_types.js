...
} from "./initialTypes/compartment_types.js";
...
CompartmentTypes.after.update(
  async function (user_id, doc, fields, modifier, options) {
    if (!doc || !modifier) return false;
    ...
    if (await is_system_admin(user_id, list)) {
      ...
      if (prev_index < current_index) {
        await CompartmentTypes.updateAsync(
          {
            $and: [
              { tabIndex: { $gt: prev_index } },
              { _id: { $ne: compart_type_id } },
              query,
            ],
          },
          { $inc: { tabIndex: current_index } },
          { multi: true },
        );
      } else {
        await CompartmentTypes.updateAsync(
    ...
    if (await is_system_admin(user_id, list)) {
      ...
      if (list["attrName"] == "elementStyle") {
        ...
        await Elements.updateAsync(
          { _id: { $in: elem_ids }, styleId: { $ne: "custom" } },
          { $set: style_update },
          { multi: true },
        );
        ...
        await CompartmentTypes.updateAsync(
          {
            $and: [
              query,
              {
                $or: [{ index: { $gt: prev_index } }, { _id: compart_type_id }],
              },
            ],
          },
