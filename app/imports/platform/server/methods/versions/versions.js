...
Versions.after.insert(async function (user_id, doc) {
  ...
  await ProjectsUsers.updateAsync(
    { userSystemId: user_id, projectId: project_id },
    { $set: { versionId: new_version_id } },
  );

  //the last published project version
  var last_version = await Versions.findOneAsync(
    { projectId: project_id, status: "Published" },
    { sort: { publishedAt: -1 } },
  );
  ...
  var users_settings = UserVersionSettings.find({
    projectId: project_id,
    versionId: last_version_id,
  });
...
Versions.after.remove(async function (user_id, doc) {
  ...
  if (last_version_id)
    await ProjectsUsers.updateAsync(
      { projectId: project_id, versionId: new_version_id },
...
async function add_read_role(proj_id, version_id) {
  ...
  for (var role in users_by_roles) {
    ...
    var reader_role = build_project_version_reader_role(
      proj_id,
      version_id,
      role,
    );
...
async function remove_from_admin_role(proj_id, version_id, is_remove_role) {
  ...
  if (is_remove_role) await Roles.deleteRoleAsync(admin_role);
}
...
async function sending_notification_email(
  notification_type,
  user_id,
  proj_name,
) {
