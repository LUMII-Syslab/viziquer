...
async function load_configurator(user_id) {
  ...
  var tool_id = await Tools.insertAsync({
    name: "_Configurator",
  ...
  var diagram_type_obj = build_ajoo_configurator_diagram_type(
    user_id,
    tool_id,
    version_id,
  );
  ...
  if ((await ElementTypes.find({ toolId: tool_id }).countAsync()) === 0) {
    var super_box_id = await build_super_box(
      tool_id,
      version_id,
      diagram_type_id,
    );
...
function build_ajoo_configurator_diagram_type(user_id, tool_id, version_id) {
  return {
    ...
    noCollectionKeyStrokes: [{ keyStroke: "Ctrl V", procedure: "Paste" }],

    globalKeyStrokes: [{ keyStroke: "Ctrl A", procedure: "SelectAll" }],
...
async function build_box_type(
  ...
  //Box specification
  var box_type_obj = build_box_type_obj(
    tool_id,
    version_id,
    diagram_type_id,
    super_box_id,
  ...
  var compart_type_obj = build_box_compart_type_obj(
    tool_id,
    version_id,
    diagram_type_id,
    box_id,
...
function build_box_compart_type_obj(
  tool_id,
  version_id,
  diagram_type_id,
  elem_type_id,
...
async function build_line_type(
  tool_id,
  version_id,
  diagram_type_id,
  super_box_id,
  ...
  //Line specification
  var edge_type_obj = build_line_type_obj(
    tool_id,
    version_id,
    diagram_type_id,
    super_box_id,
...
async function build_specialization(
  tool_id,
  version_id,
  diagram_type_id,
  super_box_id,
  ...
  var specialization_id = await ElementTypes.insertAsync({
    ...
    //key strokes
    keyStrokes: [{ keyStroke: "Delete", procedure: "Delete" }],

    //context menu
    contextMenu: [{ item: "Delete", procedure: "Delete" }],

    readModeContextMenu: [],

    //extension points
    extensionPoints: [
      { extensionPoint: "createElement", procedure: "MakeSpecialization" },
...
async function build_swimlane(tool_id, version_id, diagram_type_id) {
  var box_type_obj = {
    ...
    styles: [
      {
        id: generate_id(),
        name: "Default",
        elementStyle: { shape: "Swimlane", stroke: "black" },
      },
    ],
    ...
    //context menu
    contextMenu: [{ item: "Remove Swimlane", procedure: "RemoveSwimlane" }],
    // contextMenu: [
    // 	{item: "Cut", procedure: "Cut"},
    // 	{item: "Copy", procedure: "Copy"},
    ...
    //extension points
    extensionPoints: [
      { extensionPoint: "createElement", procedure: "MakeElementType" },
    ],
  ...
  //top row copartment
  var compart_type_obj1 = build_swimlane_compart_type_obj(
    "TopLine",
    tool_id,
    version_id,
    diagram_type_id,
  ...
  var compart_type_obj2 = build_swimlane_compart_type_obj(
    "LeftLine",
    tool_id,
    version_id,
    diagram_type_id,
  ...
  //middle compartment
  var compart_type_obj3 = build_swimlane_compart_type_obj(
    "Middle",
    tool_id,
    version_id,
    diagram_type_id,
...
function build_swimlane_compart_type_obj(
  name,
  tool_id,
  version_id,
  diagram_type_id,
