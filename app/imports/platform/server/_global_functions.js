...
async function get_user_rights_to_access_project(list, user_system_id) {
  if (list) {
    ...
    if (user_system_id) {
      var user = await ProjectsUsers.findOneAsync({
        userSystemId: user_system_id,
        projectId: proj_id,
      });
      if (user) {
          ...
          if (version) {
            ...
            //then he also has rights to access the project
              ...
              return {
                isValidUser: false,
                error:
                  "User has no rights to access the specified project version",
              };
          } else {
            return {
              isValidUser: false,
    ...
    } else return { isValidUser: false, error: "User is not logged in" };
  } else return { isValidUser: false, error: "No arguments specified" };
}
    ...
    if (search_items.length === 1) {
      ...
      query = {
        $or: [
          ...
          {
            $and: [
              { nameLC: { $regex: "^" + search_items[1] } },
              { surnameLC: { $regex: "^" + search_items[0] } },
            ],
          },
        ],
      };
    } else query = reset_query();
  }
