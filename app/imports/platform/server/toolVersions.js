...
ToolVersions.after.insert(async function (user_id, doc) {
  ...
  var user_tool = await UserTools.findOneAsync({
    userSystemId: user_id,
    toolId: doc.toolId,
  });
  if (user_tool) {
    await UserTools.updateAsync(
      { userSystemId: user_id, toolId: doc.toolId },
      { $set: { versionId: _id } },
    );
  } else {
    if (!user_id) {
  ...
  var last_version = await ToolVersions.findOneAsync(
  ...
  var compartments = Compartments.find({
    toolId: tool_id,
    versionId: last_version_id,
  });

  var diagram_types = DiagramTypes.find({
    toolId: tool_id,
    versionId: last_version_id,
  });
  var element_types = ElementTypes.find({
    toolId: tool_id,
    versionId: last_version_id,
  });
  var compartment_types = CompartmentTypes.find({
    toolId: tool_id,
    versionId: last_version_id,
  });
  var palette_buttons = PaletteButtons.find({
    toolId: tool_id,
    versionId: last_version_id,
  });
  var dialog_tabs = DialogTabs.find({
    toolId: tool_id,
    versionId: last_version_id,
  });
  ...
  for (const element_type of await element_types.fetchAsync()) {
    ...
    element_type.diagramTypeId =
      diagram_type_list[element_type.diagramTypeId];
  ...
  for (const compartment_type of await compartment_types.fetchAsync()) {
    ...
    compartment_type.diagramTypeId =
      element_list[compartment_type.diagramTypeId];
    compartment_type.elementTypeId =
      element_list[compartment_type.elementTypeId];
...
ToolVersions.after.remove(async function (user_id, doc) {
  ...
  var last_tool_version = await ToolVersions.findOneAsync(
    { toolId: tool_id },
    { sort: { createdAt: -1 } },
  );
  if (!last_tool_version) return false;
  ...
  if (user_tool)
    await UserTools.updateAsync(
      { _id: user_tool._id },
      { $set: { versionId: last_tool_version_id } },
    );
  //else
