...
Template.searchMenu.helpers({
  searches: function () {
    ...
    if (search_functions[search_type]) return search_functions[search_type]();
  },
});
...
function selecting_searches(query, limit) {
  ...
  if (!limit) limit = { $sort: { counter: -1 }, limit: 10 };
...
function search_object(e, text) {
  if (text) $("#searchField").val(text);
  else text = $("#searchField").val();
  ...
  if (search_type_func[search_type]) search_type_func[search_type]();
...
function search_keyup(e) {
  ...
  if (e.keyCode === 13) search_object(e);
  //arrow down
  else if (e.keyCode === 40) process_arrow_button("down");
  //arrow up
  ...
  //on "normal" keypress queries the DB
    ...
    if (text === "") reset_search_query(e);
    else {
      var search_type = Session.get("search_type");
      var search_type_func = {
        contacts: function () {
          var filter = { text: text, type: collection_type };
...
function process_arrow_button(arrow_type) {
  ...
  $("#searchField").val(text).attr({ itemid: item_id, phrase: text });
}
...
function compute_index(arrow_type, count, compute_index_fn) {
  ...
  if (typeof index === "undefined") {
    ...
    if (index === end_index) index = initial_value;
    else index = Number(index) + step_direction;
  }
  return index;
}

function get_display_mode(count) {
  if (count && count > 0) return "display:block;";
