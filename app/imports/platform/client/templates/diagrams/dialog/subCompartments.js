...
import { autoCompletionCleanup } from "../../../../../custom/vq/client/js/autoCompletion.js";
...
Template.multiFieldBody.events({
  "click .down-multi-field": function (e, templ) {
    ...
    if (next_index < compartments.length) {
      ...
      var list = {
        ...
        prevCompartment: {
          id: current_compart._id,
          index: current_compart.index,
        },
  ...
  "click .remove-multi-field": function (e) {
    ...
    let compartType = CompartmentTypes.findOne({
      _id: compart.compartmentTypeId,
    });
...
Template.show_multi_field_form.helpers({
  field_obj: function () {
    ...
    var compart = Compartments.findOne({
      _id: Session.get("multFieldCompartmentId"),
    });
    ...
    process_sub_compart_types(
      compart_type.subCompartmentTypes,
      fields,
      sub_compartment,
    );
...
Template.show_multi_field_form.events({
  "click .new-multi-row": function (e, templ) {
    ...
    var res = Dialog.buildSubCompartmentTree(
      multi_field,
      compart_type,
      sub_compart_tree,
    );
    ...
    if (typeof sub_compart_tree.Attributes !== "undefined") {
      ...
      if (
        typeof sub_compart_tree.Attributes.Attributes.Graph !== "undefined" &&
        typeof sub_compart_tree.Attributes.Attributes.Graph.input !==
          "undefined" &&
        sub_compart_tree.Attributes.Attributes.Graph.input !== ""
      )
        graphPrefixes = sub_compart_tree.Attributes.Attributes.Graph.input;
      if (
        typeof sub_compart_tree.Attributes.Attributes["Graph instruction"] !==
          "undefined" &&
        typeof sub_compart_tree.Attributes.Attributes["Graph instruction"]
          .input !== "undefined" &&
        sub_compart_tree.Attributes.Attributes["Graph instruction"].input !== ""
      )
        graphPrefixes =
          sub_compart_tree.Attributes.Attributes["Graph instruction"].input +
          ": " +
      ...
      if (typeof sub_compart_tree.Attributes.Attributes.Graph !== "undefined")
      ...
      if (
        typeof sub_compart_tree.Attributes.Attributes.IsInternal !==
          "undefined" &&
        sub_compart_tree.Attributes.Attributes.IsInternal.input === "true"
      )
        prefixesValue = "h";
      if (
        typeof sub_compart_tree.Attributes.Attributes["Require Values"] !==
          "undefined" &&
        sub_compart_tree.Attributes.Attributes["Require Values"].input ===
          "true"
      )
      ...
      if (
        typeof sub_compart_tree.Attributes.Attributes.Prefixes !== "undefined"
      ) {
    ...
    if (typeof src_id === "undefined") {
      Interpreter.executeExtensionPoint(compart_type, "createCompartment", [
        elem_id,
      ]);
      Interpreter.executeExtensionPoint(compart_type, "afterUpdate", [
        elem_id,
        compart_type_id,
      ]);
    }
...
function process_sub_compart_types(
  subCompartmentTypes,
  fields,
  sub_compartments,
) {
  _.each(subCompartmentTypes, function (sub_compart_type) {
    ...
    if (sub_compart_type.inputType && sub_compart_type.inputType.type) {
      var field = Dialog.renderDialogFields(
        sub_compart_type,
        sub_sub_compartments,
      );
      field.subCompartmentTypeId = sub_compart_type._id;

      field.isSubCompartmentType = true;

      var update_extension_point = _.find(
        sub_compart_type.extensionPoints,
        function (ext_point) {
    ...
    if (
      ...
      sub_compart_type.subCompartmentTypes.length > 0
    ) {
      process_sub_compart_types(
        sub_compart_type.subCompartmentTypes,
        fields,
...
function getCurrentCompartment(e) {
  ...
  var compartments = Compartments.find(
    {
  ...
  return {
    ...
    index: index,
    currentCompartment: current_compart,
  };
}
...
function get_multi_fields_obj() {
  ...
  var compartments = Compartments.find(
    { compartmentTypeId: compart_type_id, elementId: elem_id },
    { sort: { index: 1 } },
  ).fetch();
