...
Template.dialogTabContent.events({
  ...
  "keydown .dialog-input, keydown .dialog-combobox": function (e) {
    ...
    if (!compart_type) {
      ...
      if (parent_compart_type) {
        compart_type = find_compart_type_object(
          parent_compart_type.subCompartmentTypes,
          compart_type_id,
        );
      }
    }
  ...
  "click .attach-file-btn": function (e) {
    e.preventDefault();
    var form = $(e.target)
      .closest(".attach-container")
      .find(".attach-file-form");
...
Template.attachFiles.events({
  "click .attach-file": function (e) {
    ...
    var list = {
      ...
      index:
        DiagramFiles.find({ elementId: Session.get("activeElement") }).count() +
        1,
    };
...
function update_combobox(e) {
  ...
  if (parent.length > 0) {
    ...
    var selected = src
      .closest(".compart-type")
      .find('option[input="' + src_val + '"]');
    var mapped_value = selected.attr("mappedValue");
...
function define_dialog_tab_template(id) {
  var template_structure =
    '{{> Template.dynamic template="dialogTabContent" data=compart_types}}';
...
function add_template_helpers(id) {
  ...
  Template[id].helpers({
    compart_types: function () {
      return CompartmentTypes.find(
        { dialogTabId: id },
        { sort: { tabIndex: 1 } },
      ).map(function (compart_type) {
        var compartment = Compartments.findOne({
          compartmentTypeId: compart_type._id,
          elementId: Session.get("activeElement"),
        });

        var is_visible = check_compartment_visibility(
          compart_type,
          compartment,
        );
        if (is_visible) {
...
function build_sub_compartment_tree(parent, compart_type, compart_tree) {
  ...
  _.each(sub_compart_types, function (sub_compart_type, i) {
    ...
    if (sub_sub_compart_types.length === 0) {
      ...
      var value = build_compartment_value(
        sub_compart_type,
        input_value,
        mapped_value,
      );
      sub_compart_tree[sub_compart_type.label] = {
        value: value,
        input: input_value,
      };
      res.push(value);
    } else {
      var val = build_sub_compartment_tree(
        parent,
        sub_compart_type,
        sub_compart_tree,
      );
      if (val) {
        if (sub_compart_type.prefix) val = sub_compart_type.prefix + val;

        if (sub_compart_type.suffix) val = val + sub_compart_type.suffix;
      }
    ...
    if (len > 1 && i < len - 1) res.push(concat_style);
  });
  ...
  Template[id].helpers({
    compart_types: function () {
      return CompartmentTypes.find(
        ...
        var is_visible = Dialog.checkCompartmentVisibility(
          compart_type,
          compartment,
        );
        if (is_visible) {
...
function upsert_compartment_value(
  e,
  elem_id,
  src_id,
  src_val,
  ...
  Interpreter.executeExtensionPoint(compart_type, "beforeUpdate", [
    src_id,
    src_val,
  ]);
  var update_res =
    Interpreter.executeExtensionPoint(compart_type, "update", [
      elem_id,
      src_id,
      src_val,
      mapped_value,
  ...
  //after
  var after_update = Interpreter.getExtensionPointProcedure(
    "afterUpdate",
    compart_type,
  );
  if (after_update) {
...
function find_compart_type_object(sub_compart_types, compart_type_id) {
  for (var i = 0; i < sub_compart_types.length; i++) {
    ...
    if (_.size(sub_compart_type.subCompartmentTypes) > 0) {
      return find_compart_type_object(
