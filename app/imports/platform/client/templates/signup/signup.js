...
Template.signup.helpers({
  ...
  button_enabled: function () {
    if (Session.get("IsSignUpEnabled")) return "";
...
function blur_password_fields(e) {
  ...
  if (field.hasClass("parsley-validated")) return;
...
function process_fields(field, var_name, field_name) {
  if (!field.val()) Session.set(var_name, field_name + " is required.");
  else Session.set(var_name, reset_variable());
}
...
function check_fields_values() {
  ...
  if (
    ...
    Session.get("confirmedPasswordError") ||
    Session.get("name_error") ||
    Session.get("surname_error") ||
    Session.get("secret_phrase_error") ||
    !email ||
    !password ||
    !confirmed_password ||
    !name ||
