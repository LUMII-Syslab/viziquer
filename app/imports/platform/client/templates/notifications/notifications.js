...
Template.newNotificationsCountT.helpers({
  newNotificationsCount: function () {
    return Notifications.find({
      status: "new",
      receiver: Session.get("userSystemId"),
    }).count();
  },
});
...
function accept_invitation(e) {
  ...
  Utilities.callMeteorMethod("updateNotification", list, function () {
    ...
    if (proj_id) {
      var proj_user = ProjectsUsers.findOne({
        projectId: proj_id,
...
function process_notifications(notifications_cursor) {
  return notifications_cursor.map(function (notification) {
    ...
    if (
      ...
      else if (notification.type === "Removed") notification.removed = true;
      else if (notification.type === "ChangeRole") notification.changed = true;

      if (notification.status === "seen") notification.seen = true;
      else if (notification.status === "confirmed")
        notification.confirmed = true;
      else if (notification.status === "rejected") notification.rejected = true;
    }
