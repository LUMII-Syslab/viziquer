...
import { analytics } from "../../../../client/lib/global_variables.js";
...
Template.basicInfoPanel.events({
  ...
  "blur .profile-field": function (e) {
    var field = $(e.target);
    if (field.attr("type") === "text") var update = {};
...
Template.passwordPanel.helpers({
  old_password_error: function () {
    if (Session.get("oldPasswordError")) return Session.get("oldPasswordError");
    else return {};
  },

  new_password_error: function () {
    if (Session.get("newPasswordError")) return Session.get("newPasswordError");
    else return {};
  },
...
Template.passwordPanel.events({
  ...
  "keyup #oldPassword": function (e) {
    ...
    if (field.hasClass("parsley-validated")) {
      var val = field.val();
      if (val.length > 0) Session.set("oldPasswordError", {});
      else
        Session.set("oldPasswordError", {
          errorClass: get_error_class(),
    ...
    if (field.hasClass("parsley-validated")) {
      if (field.attr("id") === "newPassword") {
        ...
        if (new_password === "") {
          ...
          //if the password is too weak
          if (new_password_status.status) Session.set("newPasswordError", {});
          else
            Session.set("newPasswordError", {
              errorClass: get_error_class(),
              message: new_password_status.message,
        ...
        if (confirmed_password === "") {
          ...
          } else Session.set("confirmedPasswordError", {});
        }
      }
...
function check_password_complexity(password) {
  if (password) {
    if (password.length > 7) return { status: true };
    else return { status: false, message: "PasswordTooShort" };
  } else return { status: false, message: "EmptyFieldError" };
}
...
function initialize_user_tag_field(tags) {
  ...
  if (tags_field.length > 0 && !tags_field.attr("initialized")) {
    ...
    var tags_input = $(".bootstrap-tagsinput").css({
      width: "100%",
      "margin-bottom": 0,
    });
