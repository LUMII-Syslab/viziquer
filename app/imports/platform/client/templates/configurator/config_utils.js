...
var Configurator = {
  ...
  getActiveElementExtension: function (extension_point_name) {
    var element_type = ElementTypes.findOne({
      elementId: Session.get("activeElement"),
    });
    if (element_type) {
      var extension_point = _.find(
        element_type.extensionPoints,
  ...
  getActiveCompartmentType: function () {
    return CompartmentTypes.findOne({
      _id: Session.get("compartmentTargetTypeId"),
    });
  },
  ...
  deleteKeystrokeOrItem: function (e, procedure_name, collection) {
    ...
    var table_rows = _.filter(
      target_cell.closest(".table-body").children(),
      function (row) {
  ...
  getKeystrokesOrItems: function (collection_type) {
    var items;
    if (Session.get("activeElement")) {
      var target_elem_type = ElementTypes.findOne({
        elementId: Session.get("activeElement"),
      });
      if (target_elem_type) {
      ...
      var taget_diagram_type = DiagramTypes.findOne({
        _id: Session.get("targetDiagramType"),
      });
  ...
  updateKeystrokeOrItem: function (e, procedure_name, collection) {
    ...
    var field_update = {};
    field_update[collection.array + "." + index + "." + collection.field] =
      new_value;
...
function update_element_style_object(attr_name, attr_value, style_type) {
  var list = { attrName: style_type + "." + attr_name, attrValue: attr_value };
