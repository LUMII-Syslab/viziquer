...
Template.inputOptions.events({
  ...
  "blur .placeholder": function (e) {
    ...
    var list = {
      attrName: "inputType.placeholder",
      attrValue: placeholder_val,
    };
...
Template.checkboxTable.helpers({
  items: function () {
    ...
    return _.map(values, function (item) {
      ...
      item.compartmentStyles = select_element_type_styles(
        compart_type,
        values[i],
      );
    ...
    if (tab_name && tab_name !== "") {
      ...
      if (Session.get("activeElement")) {
        ...
        dialog_tab = DialogTabs.findOne(
          { elementTypeId: elem_type._id, elementTypeId: { $exists: true } },
          { sort: { index: 1 } },
        );
      } else {
        dialog_tab = DialogTabs.findOne(
...
function get_style_list(obj_type, value, attr) {
  ...
  _.each(obj_type.styles, function (style, i) {
    ...
    if (value[attr] === item.id) item.selected = "selected";
