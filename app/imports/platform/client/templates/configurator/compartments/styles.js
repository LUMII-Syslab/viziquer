...
Template.compartmentStyleContextMenu.helpers({
  is_remove: function () {
    if (Session.get("activeCompartmentStyleIndex") === 0) return "disabled";
  },
});
  ...
  styles: function () {
    ...
    if (compart_type) {
      ...
      return _.map(compart_type.styles, function (style, i) {
        style.index = i;
        if (i === index) style.selected = "selected";
...
function get_compartment_type_default_style() {
  ...
  if (compart_type && compart_type.styles) {
    ...
    if (is_ajoo_editor(editor_type)) {
      ...
      set_selection_value(style, "align", [
        { value: "left" },
        { value: "center" },
        { value: "right" },
      ]);

      set_selection_value(style, "fontStyle", [
        { value: "normal" },
        { value: "bold" },
        { value: "italic" },
      ]);

      set_selection_value(style, "fontVariant", [
        { value: "normal" },
        { value: "small-caps" },
      ]);

      set_selection_value(style, "visible", [
        { value: "true" },
        { value: "false" },
      ]);
...
function add_compartment_style(list) {
  Utilities.callMeteorMethod("addCompartmentTypeStyle", list, function () {
    ...
    if (compart_type && compart_type.styles) {
      Session.set(
        "activeCompartmentStyleIndex",
        compart_type.styles.length - 1,
      );
    }
  });
}
