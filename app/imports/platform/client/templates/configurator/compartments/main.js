...
Template.compartmentsAccordion.rendered = function () {
  if (Session.get("compartmentTargetTypeId")) {
    $("option#" + Session.get("compartmentTargetTypeId")).prop(
      "selected",
      "selected",
    );
  } else {
...
Template.compartmentMain.events({
  "click #addCompartment": function (e) {
    ...
    if (Session.get("activeElement")) {
      ...
      compart_types_count = CompartmentTypes.find({
        elementTypeId: list.elementTypeId,
      }).count();

      //selecting the last compartment type
      last_compart_type = CompartmentTypes.findOne(
        { elementTypeId: list.elementTypeId },
        { sort: { tabIndex: -1 } },
      );

      //selecting the tab
      tab = DialogTabs.findOne(
        { elementTypeId: elem_type._id, type: { $exists: false } },
        { sort: { index: -1 } },
      );
    }
    ...
    //if the new compartment type is attached to the diagram type
      ...
      //selecting the tab
      tab = DialogTabs.findOne(
        { elementTypeId: { $exists: false }, type: { $exists: false } },
        { sort: { index: -1 } },
      );
    }
