...
Template.timeline.helpers({
  versions: function () {
    ...
    return ToolVersions.find({}, { sort: { createdAt: -1 } }).map(
      function (version) {
        //transforms the published date in different form
        var date = version.publishedAt;
        if (date) version.date = joined_date(date);

        //sets active version
        if (version._id === active_version) version.active = true;
...
Template.timeline.events({
  "click .pull": function (e) {
    ...
    Meteor.call("upsertUserTool", list, function (err) {
      if (err) console.log("Error in upsertTool callback", err);
    });
  ...
  buttons: function () {
    return {
      ...
      remove_version_disabled: function () {
        ...
        if (version_count > 1) return is_publish_bottom_disabled();
        else return true;
      },
  ...
  "click #new-version": function (e) {
    ...
    if (!new_version) {
      ...
      Meteor.call("newToolVersion", list, function (err) {
        if (err) console.log("Error in newToolVersion callback", err);
        else {
          if (version_id)
    ...
    if (new_version) {
      ...
      if (user_tools) {
        if (user_tools.versionId === version_id) {
          var last_version = ToolVersions.findOne(
            {},
            { sort: { createdAt: -1 } },
          );
          if (last_version)
            FlowRouter.go("tool", {
              _id: Session.get("toolId"),
              versionId: last_version._id,
            });
          else FlowRouter.go("tool", { _id: Session.get("toolId") });
        }
      }
    }
  },
  ...
  "click #publish": function (e, templ) {
    ...
    if (new_version) {
      ...
      Meteor.call("publishToolVersion", list, function (err) {
        if (err) console.log("Error in publishToolVersion callback", err);
      });
    }
  },
});
...
function is_publish_bottom_disabled() {
  var version = ToolVersions.findOne({
    toolId: Session.get("toolId"),
    status: "New",
  });
