  ...
  tabs: function () {
    ...
    if (dialog_tabs) {
      return dialog_tabs.map(function (dialog_tab) {
        var rows = CompartmentTypes.find(
          { dialogTabId: dialog_tab._id },
          { sort: { tabIndex: 1 } },
        );
        if (rows.count() > 0) dialog_tab._rows = rows;
        else dialog_tab._delete = true;
    ...
    stop: function (event, ui) {
      ...
      if (el.hasClass("compartment")) {
        ...
        if (tab.attr("type")) {
          ...
          Meteor.call(
            "reorderCompartmentTypeTabIndexes",
            params,
            function (err) {
              if (err) {
    ...
    stop: function (event, ui) {
      ...
      if (el.hasClass("tab")) {
        ...
        Meteor.call("reorderTabIndexes", params, function (err) {
          if (err) console.log("Error in reorderTabIndexes callback", err);
        });
      }
    },
  });
...
Template.newTab.events({
  "click #add-tab": function (e) {
    ...
    if (elem_type) {
      ...
      let dialog_tab = DialogTabs.findOne(
...
function update_tab_name(id, name) {
  var list = { tabId: id, toolId: Session.get("toolId"), name: name };
