...
Template.membersFilter.events({
  ...
  "keyup #findUser": function (e) {
    ...
    var query;
    if (search_entered !== "") query = build_user_search_query(search_entered);
...
Template.userGroups.events({
  ...
  "blur .edit-group-name": function (e) {
    ...
    if (group) {
      if (group.name !== value) input.text("");
    }
...
Template.foundUsers.helpers({
  foundUsers: function () {
    ...
    var user_ids = ProjectsUsers.find({
      projectId: Session.get("activeProject"),
    }).map(function (proj_user) {
...
function get_project_members() {
  ...
  if (members_filter) {
    ...
    if (user_ids.length > 0) filter_query = { systemId: { $in: user_ids } };
  ...
  return ProjectsUsers.find({ projectId: Session.get("activeProject") }).map(
    function (proj_user, i) {
      ...
      if (filter_query)
        user_query = {
          $and: [{ systemId: proj_user.userSystemId }, filter_query],
        };
      else user_query = { systemId: proj_user.userSystemId };
...
function build_user_search_query(search_entered) {
  ...
  var query = {};
  if (search_items.length === 0) query = {};
  else {
    //filters by name, surname or email
    if (search_items.length === 1) {
      query = {
        $or: [
          { name: { $regex: "^" + search_items[0], $options: "i" } },
      ...
      query = {
        $or: [
          ...
          {
            $and: [
              { name: { $regex: "^" + search_items[1], $options: "i" } },
              { surname: { $regex: "^" + search_items[0], $options: "i" } },
            ],
          },
        ],
      };
    } else query = Utilities.resetQuery();
  }
...
function render_members() {
  ...
  return _.map(members, function (member) {
    ...
    if (member) member.groups = groups;

    if (is_default_role(member.role)) member.defaultRole = true;
    else {
      var group_id = member.role;
      var group = ProjectsGroups.findOne({ _id: group_id });
      if (group) member.roleName = group.name;
    }
    ...
    if (meteor_user) {
      ...
      if (status) {
        if (status.idle) member.idle = true;
        else if (status.online) member.online = true;
...
function is_default_role(role) {
  if (role === "Admin" || role === "Reader") return true;
}
