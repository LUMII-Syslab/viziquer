...
const Utilities = {
  isAdmin: function () {
    var user = ProjectsUsers.findOne({
  ...
  isEditable: function () {
    ...
    if (user) {
      var role = build_project_version_admin_role(
        Session.get("activeProject"),
        Session.get("versionId"),
      );
  ...
  editUserProfile: function (update, operation) {
    var list = { update: update, operation: operation };
  ...
  changeUserActiveProject: async function (proj_id) {
    ...
    if (proj_user) {
      ...
      Utilities.editUserProfile({
        activeProject: proj_id,
      ...
      if (proj !== undefined) {
        ...
        if (tool.toolGroup !== undefined) {
          ...
          Interpreter.executeExtensionPoint(
            {
              extensionPoints: [
                { extensionPoint: "changeProject", procedure: "changeProject" },
              ],
            },
            "changeProject",
            proj_id,
  ...
  callMeteorMethodAsync: async function (method_name, ...args) {
    try {
      if (args.length > 0 && typeof args[args.length - 1] === "function") {
        console.error(
          method_name,
          "callback functions not (yet) supported in async mode",
        );
        return null;
      } else {
