  ...
  var elem_handle = Elements.find({
    ...
    added: function (id, elem) {
      ...
      if (init) {
        if (elem.type === "Box") {
          boxes.push(elem);
        } else if (elem.type === "Line") lines.push(elem);

        var comparts_query = build_comparts_query();
        _.extend(comparts_query, { elementId: elem._id });
        elem.compartments = Compartments.find(comparts_query, {
          sort: { index: 1 },
        }).fetch();
      }
      ...
      //if added later
        ...
        if (elem.type === "Box") data.boxes.push(elem);
        else if (elem.type === "Line") data.lines.push(elem);
      ...
      if (fields.style) {
        ...
        if (element.type === "Box") {
          ...
          if (style_obj.shape && element.name !== style_obj.shape) {
            ...
            if (selected[id]) is_selected = true;
    ...
    removed: function (id) {
      ...
      if (swimlane_layer) swimlane_layer.batchDraw();
  ...
  var compart_handle = Compartments.find(comparts_query).observeChanges({
    ...
    changed: function (id, fields) {
      ...
      if (fields.style) {
        ...
        if (element.type === "Line") {
          if (fields.style.placement) {
            var placement = compartments.getPlacementByName(
              fields.style.placement,
            );
      ...
      if (fields.index) {
        ...
        var comparts_in = Compartments.find(
...
function build_palette_button(id, palette_button) {
  ...
  var elem_type = ElementTypes.findOne({
    _id: { $in: palette_button.elementTypeIds },
  });
  if (!elem_type) {
  ...
  if (elem_type.lineType) complex_style.lineType = elem_type.lineType;
  ...
  if (elem_type.defaultFixedSize) {
    ...
    if (elem_type.defaultFixedSize.defaultFixedWidth)
      palette_button.defaultSize.width =
        elem_type.defaultFixedSize.defaultFixedWidth;

    if (elem_type.defaultFixedSize.defaultFixedHeight)
      palette_button.defaultSize.height =
        elem_type.defaultFixedSize.defaultFixedHeight;
  }
...
function compute_palette() {
  var palette = PaletteButtons.find({
    diagramTypeId: Session.get("diagramType"),
  }).map(function (palette_button) {
