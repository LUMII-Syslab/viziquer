...
Interpreter.loadAjooEditor = function (diagram) {
  ...
  if (
    diagram.editing &&
    diagram.editing.userId === Session.get("userSystemId")
  ) {
    is_edit_mode = true;
  }
  ...
  var container_height =
    $(window).height() - $("#" + container_name).offset().top - 200;
...
function get_event_functions() {
  return {
    ...
    rClickedOnDiagram: function (data) {
      ...
      if (editor.isSelectionEmpty()) {
        ...
        if (Session.get("editMode")) {
          menu = get_context_menu_list(
            diagram_type,
            "noCollectionContextMenu",
            "dynamicNoCollectionContextMenu",
          );
        } else {
          menu = get_context_menu_list(
            diagram_type,
            "readModeNoCollectionContextMenu",
            "dynamicReadModeNoCollectionContextMenu",
          );
        }
    ...
    rClickedOnCollection: function (data) {
      ...
      if (Interpreter.editor.isEditMode()) {
        ...
        menu = get_context_menu_list(
          ...
          "dynamicReadModeCollectionContextMenu",
        );
      }
    ...
    newElementStarted: function (data) {
      ...
      var comparts_with_defaults = _.filter(
      ...
      data.element.compartments = _.map(
        comparts_with_defaults,
        function (compart_type) {
...
function is_new_line_allowed(state) {
  if (state.end) {
    ...
    var line_type = compute_new_line_type(
      [elem_type_id],
      start_elem_id,
      end_elem_id,
    );
