import {
  ...
  ACircle,
  Arrow,
} from "../Boxes/DrawingShapes/shapes1";
...
LineEndShape.prototype.create = function (style) {
  ...
  if (shape_name && shape_name !== "None") {
    ...
    if (!style.width) style.width = style.radius;

    if (!style.height) style.height = style.radius;
...
LineEndShape.prototype.addEndShapeRepresentationElement = function (style) {
  ...
  var end_shapes = {
    ...
    Arrow: function () {
      ...
      Arrow.prototype.updateShapeSize.call(self, {
        width: style.width,
        height: style.height,
      });
...
LineEndShape.prototype.computeLineEndShape = function () {
  ...
  _.each(shapes, function (shape) {
    ...
    var rotation = line_end.computeRotation(
      points,
      x1,
      y1,
      x2,
    ...
    var center = line_end.moveCenter(
      shape,
      points,
      end_point_x,
      end_point_y,
...
LineEndShape.prototype.computeRotation = function (
  points,
  x1_index,
  y1_index,
  x2_index,
...
LineEndShape.prototype.moveCenter = function (
  shape,
  points,
  end_point_x_index,
  end_point_y_index,
  ...
  var move_func;
  if (shape.className === "Line") move_func = "moveArrowCenter";
