...
FlowNode.prototype.getName = function () {
  return this.parent.compartments.length > 0
    ? this.parent.compartments[0].value
    : "";
};
...
FlowPath.prototype.otherNode = function (nodeId) {
  if (this.from === nodeId) return this.owner.idToObject[this.to];
  else return this.owner.idToObject[this.from];
};
//******************************************************************************
// FlowGraph
//******************************************************************************
...
FlowGraph.prototype.fromSide = function () {
  if (this.routingStyle === 0 && this.layoutStyle === 0)
    if (this.mode === 0) return 8;
    else if (this.mode === 1) return 4;
    else if (this.mode === 2) return 2;
    else return 1;
  else return 15;
};
FlowGraph.prototype.toSide = function () {
  if (this.routingStyle === 0 && this.layoutStyle === 0)
    if (this.mode === 0) return 2;
    else if (this.mode === 1) return 1;
    else if (this.mode === 2) return 8;
    else return 4;
  else return 15;
};
      ...
      _.each(nodes, function (node) {
        _.each(node.paths, function (path) {
          ...
          if (node.level + 1 === other.level) {
            ...
            console.log(
              "FlowGraph.prototype.reroute wrong levels of path ",
              path.id,
              " from level ",
              node.level,
              " to level ",
      ...
      _.each(turns, function (sideTurns, i) {
        ...
        _.each(sideTurns, function (turn, j) {
          str +=
            ...
            ", max " +
            turn.max;
          if (j > 0) str += ", cross " + turn.cross(sideTurns[j - 1]);
        });
      });
...
FlowGraph.prototype.getTurns = function (paths) {
  ...
  _.each(paths, function (path, i) {
    turns.push(new TurnInfo(path, dir));
  });
  return turns;
};
FlowGraph.prototype.routeDummyOrthogonal = function () {
  ...
  _.each(this.paths, function (path, i) {
    ...
    sz = [
      (source.size[0] - target.size[0]) / 2,
      (source.size[1] - target.size[1]) / 2,
    ];
    if (dir === 0)
      line.points = [
        source.center[0] + (sng * source.size[0]) / 2,
        source.center[1],
        (source.center[0] + target.center[0] - sz[0]) / 2,
        source.center[1],
        (source.center[0] + target.center[0] - sz[0]) / 2,
        target.center[1],
        target.center[0] - (sng * target.size[0]) / 2,
        target.center[1],
      ];
    else
      line.points = [
        source.center[0],
        source.center[1] + (sng * source.size[1]) / 2,
        source.center[0],
        (source.center[1] + target.center[1] - sz[1]) / 2,
        target.center[0],
        (source.center[1] + target.center[1] - sz[1]) / 2,
        target.center[0],
        target.center[1] - (sng * target.size[1]) / 2,
      ];
  });
};
...
FlowGraph.prototype.markAddingObjects = function () {
  ...
  _.each(this.parent.lines, function (line) {
    id = line._id;
    if (
      self.idToObject[id] === undefined &&
      (self.linkTypes === undefined ||
        (self.idToObject[id] === undefined &&
          self.linkTypes[line.elementTypeId] !== undefined))
    ) {
      if (self.linkTypes[line.elementTypeId] === true)
        self.markAddingLine(line);
      else self.splitForkLine(line);
    }
  });
};
...
FlowGraph.prototype.addNodes = function () {
  ...
  _.each(this.parent.boxes, function (box) {
    if (self.idToObject[box._id]) self.addNode(box);
  });
};
...
FlowGraph.prototype.addPaths = function () {
  var self = this;
  _.each(this.parent.lines, function (line) {
    if (self.idToObject[line._id]) self.addPath(line);
  });
};
...
FlowGraph.prototype.compareSplitLine = function (line) {
  ...
  if (v.level !== w.level)
    if (v.level < w.level) return [-1, v.level, w.level];
    else return [1, w.level, v.level];
  if (v.fixed === w.fixed) return [0, v.level, w.level];
  else if (v.fixed) return [-1, v.level, w.level];
  else return [1, w.level, v.level];
};
...
FlowGraph.prototype.createUnflow = function () {
  ...
  _.each(this.parent.lines, function (line) {
    ...
    if (path === undefined && line.startElement !== line.endElement) {
      ...
      if (!v.fixed) v.paths.push(w);
      if (!w.fixed) w.paths.push(v);
    }
  });
};
...
FlowGraph.prototype.setLevels = function (nodes) {
  ...
  _.each(nodes, function (node) {
    ...
    if (levels.length < 1) {
      ...
      level = _.find(levels, function (lev) {
        return Math.abs(lev - c) < 2;
      });
      if (level === undefined) {
        levels.push(c);
        //                console.log("\t\tnew  level", c, node.id);
      } else node.center[dir] = level;
...
FlowGraph.prototype.testUnknownOWLTypes = function () {
  var unknownTypes = _.uniq(
    _.union(
      this.checkOWLTypes(this.parent.boxes),
...
FlowGraph.prototype.setResultDiagram = function () {
  ...
  if (!_.isEmpty(this.removed)) {
    ...
    _.each(self.parent.lines, function (line) {
      if (self.removed[line._id] !== true) newLines.push(line);
    });
...
FlowGraph.prototype.arrangeLayout = function (nodes, incrementally) {
  ...
  _.each(nodes, function (node, i) {
    if (added[node.id] === undefined) {
      ...
      if (node.fixed) {
        layout.addBox(
          node.id,
          node.center[0] - node.size[0] / 2,
          node.center[1] - node.size[1] / 2,
          node.size[0],
          node.size[1],
        );
    ...
    _.each(node.paths, function (path) {
      if (added[path.id] === undefined) {
        if (added[path.from] === undefined || added[path.to] === undefined)
          warning(
            "FlowGraph.prototype.arrangeLayout path end nodes are not added to layout graph.",
          );

        if (OWLTypes[path.parent.elementTypeId].type === "flow")
          path.parent.style.lineType = LineTypes[self.routingStyle];
        else path.parent.style.lineType = "Direct";
        ...
        options = {
          lineType: lineStyle,
          startSides: self.fromSide(),
          endSides: self.toSide(),
        };
  ...
  console.log(
    "\nArrangeFromScratch graph component with ",
    this.nodes.length,
    " nodes and ",
    this.paths.length,
    " links.",
  ...
  else return layout.arrangeFromScratch();
};
...
FlowGraph.prototype.getLevelStr = function (nodes, text) {
  ...
  _.each(nodes, function (node, i) {
    ...
    _.each(node.paths, function (path, i) {
      v =
        path.rom === node.id
          ? self.idToObject[path.to]
          : self.idToObject[path.from];
      if (v !== undefined) str += " " + v.id + ", " + v.level + ";";
    });
    str += "]";
  });
  return str;
...
FlowGraph.prototype.getDiagramStr = function (text) {
  ...
  _.each(this.parent.lines, function (line) {
    ...
    q = line.fixed ? "*" : " ";
    boxInfo[line.startElement] +=
      " " +
      q +
      OWLTypes[line.elementTypeId].name +
      "<" +
      r +
      line.endElement +
      ";";
    boxInfo[line.endElement] +=
      " " +
      q +
      OWLTypes[line.elementTypeId].name +
      r +
...
function getDiagranNodesStr(text, boxes) {
  ...
  _.each(boxes, function (box) {
    str +=
      ...
      box.location.y +
      "]s[" +
      (box.location.x + box.location.width / 2) +
      "," +
      (box.location.y + box.location.height / 2) +
      "])" +
      "]c[" +
      box.location.width +
      "," +
...
function my_getNodeAndLinkTypes(diagrams) {
  ...
  _.each(diagrams, function (obj) {
    ...
    _.each(diagram.nodes, function (node) {
      if (nodeTypes[node.type] === undefined) nodeTypes[node.type] = 0;
      nodeTypes[node.type]++;
    });
    _.each(diagram.edges, function (line) {
      if (lineTypes[line.type] === undefined) lineTypes[line.type] = 0;
      lineTypes[line.type]++;
    });
  });
  let str = "nodeTypes:\n";
...
var LayoutStyles = [
  ["INVERSE_HORIZONTAL", "VERTICAL", "HORIZONTAL", "INVERSE_VERTICAL"],
  ["SYMMETRIC"],
  ["UNIVERSAL"],
];
...
var FlowLinkTypes = {
  "OWL.AssocToFork": true,
  "OWL.Generalization": false,
  "OWL.GeneralizationToFork": true,
};
var SplitLinkTypes = {
  "OWL.Association": "dummy",
  "OWL.Connector": "dummy",
  "OWL.Dependency": "dummy",
  "OWL.Disjoint": "dummy",
  "OWL.Link": "dummy",
  "OWL.Restriction": "dummy",
  "OWL.EquivalentClass": "dummy",
};
...
TurnInfo.prototype.compare = function (other) {
  ...
  // intersect intervals
    ...
    if (this.min < other.min && this.max < other.max) return -1;
    else if (this.min > other.min && this.max > other.max) return 1;
    // turns has crossing
    else if (this.min < other.min && this.max > other.max) return -1;
    else return 1;
  else if (this.min > other.min && this.max > other.max) return 1;
  else if (this.min < other.min && this.max < other.max) return -11;
  // turns has crossing
  else if (this.min > other.min && this.max < other.max) return 1;
  else return -1;
};
