...
import { OrthogonalCollectionRerouting } from "../Elements/Lines/routing/orthogonal_rerouting.js";
import { LineSVGObject } from "../Elements/Lines/routing/svg_collisions.js";
...
var SelectionDragging = function (editor) {
  ...
  selection_dragging.dragGroup = editor.findChild(
    selection_dragging.dragLayer,
    "DragGroup",
  );

  selection_dragging.inLinesGroup = editor.findChild(
    selection_dragging.dragLayer,
    "InLinesGroup",
  );
  selection_dragging.outLinesGroup = editor.findChild(
    selection_dragging.dragLayer,
    "OutLinesGroup",
  );
...
SelectionDragging.prototype = {
  ...
  selectDraggedElements: function (list) {
    ...
    _.each(selection_dragging.state.draggedLines, function (line_obj) {
      ...
      list.lines.push({ id: link._id, points: line_points });
  ...
  adjustDragGroupToGrid: function (param) {
    ...
    if (editor.grid.isGridEnabled) {
      ...
      if (_.size(this.state.draggedPorts) > 0) {
        ...
        var deltas = [
          { name: "deltaX1", value: Math.abs(-new_x + x1) },
    ...
    return {
      deltaX: Math.round(new_delta_x),
      deltaY: Math.round(new_delta_y),
      isMoved: isMoved,
    };
  },
  ...
  buildGraphInfo: function (delta_obj) {
    ...
    OrthogonalCollectionRerouting.recomputeLines(
      editor,
      state.draggedBoxes,
      state.draggedPorts,
      state.linkedPorts,
      state.linkedOrthogonalLines,
      state.draggedLines,
      delta_obj.deltaX,
      delta_obj.deltaY,
    );
  ...
  recomputeDirectLines: function (delta_obj) {
    ...
    _.each(state.linkedDirectLines, function (line_obj) {
      ...
      var new_start_point_obj = line_svg.getIntersectionWithElement(
        start_elem,
        [points[2], points[3]],
      );
      if (!new_start_point_obj.point) {
        ...
        new_start_point_obj = line_svg1.getIntersectionWithElement(start_elem, [
          start_line[2],
          start_line[3],
        ]);
      }
      ...
      var new_end_point_obj = line_svg.getIntersectionWithElement(end_elem, [
        points[0],
        points[1],
      ]);
      ...
      if (!new_end_point_obj.point) {
        ...
        new_end_point_obj = line_svg2.getIntersectionWithElement(end_elem, [
          end_line[0],
          end_line[1],
        ]);
      }
      ...
      if (new_start_point_obj.point && new_end_point_obj.point) {
        line.setPoints([
          new_start_point_obj.point[0],
          new_start_point_obj.point[1],
          new_end_point_obj.point[0],
          new_end_point_obj.point[1],
        ]);
      }
    });
  },
