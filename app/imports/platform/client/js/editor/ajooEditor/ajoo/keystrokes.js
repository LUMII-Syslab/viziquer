...
var KeyStrokes = function (e) {
  ...
  if (e.keyCode === 17 || Session.get("editingDialog")) return false;
  ...
  if (
    ...
    if (editor.isSelectionEmpty()) {
      if (is_editor_in_edit_mode())
        is_procedure_executed = process_keystroke(
          e,
          diagram_type.noCollectionKeyStrokes,
        );
      else
        is_procedure_executed = process_keystroke(
          e,
          diagram_type.readModeNoCollectionKeyStrokes,
        );
    } else {
      ...
      //if selection size is greater than 1, then executes function from collectionKeyStrokes array
        ...
        if (is_editor_in_edit_mode())
          is_procedure_executed = process_keystroke(
            e,
            diagram_type.collectionKeyStrokes,
          );
        else
          is_procedure_executed = process_keystroke(
            e,
            diagram_type.readModeCollectionKeyStrokes,
          );
      }
    }
...
function process_keystroke(e, key_strokes) {
  ...
  for (var i = 0; i < encoded_key_strokes.length; i++) {
    ...
    if (
      ...
      (keyStroke.keys.keyCode === key_code ||
        keyStroke.keys.keyCode === key_code + 32)
    ) {
...
function encode_keystrokes(key_strokes) {
  ...
  $.each(key_strokes, function (index, key_stroke) {
    ...
    else if (keys[0] === "Delete")
      //delete has a special key code
      item.keys = { ctrl: false, keyCode: 46 };
    //"standart" key stroke
    else item.keys = { ctrl: false, keyCode: keys[0].charCodeAt(0) };
