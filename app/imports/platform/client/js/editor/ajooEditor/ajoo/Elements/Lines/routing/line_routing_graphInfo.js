...
GraphInfo.prototype.addBox = function (info) {
  if (this.infoDataMap[info.id]) return;
  ...
  _.each(this.dragObjects, function (box) {
    if (box.paths.length > 0) {
      ...
      _.each(box.paths, function (path) {
        if (path.from === box.id) path.cutPointInToBox();
        else path.cutPointInFromBox();
        //                path.avoidPathCrossings();
        if (!path.hasFromProj() || !path.hasToProj()) path.simplify();
      });
    }
  });
};
...
GraphInfo.prototype.avoidPathCrossings = function () {
  ...
  for (var i = 0; i < this.affectedPaths.length; i++) {
    ...
    if (!path.isSelfloop()) {
      if (this.isDraggedBox[path.from]) path.removeGaps(1);
      else if (this.isDraggedBox[path.to]) path.removeGaps(path.n - 1);
      path.avoidCrossings();
    }
  }
...
GraphInfo.prototype.collectDisconnectedPaths = function () {
  ...
  for (var i = 0; i < this.affectedPaths.length; i++) {
    var path = this.affectedPaths[i];
    if (!path.hasFromProj())
      if (rc[path.from] === undefined) rc[path.from] = [path];
      else rc[path.from].push(path);
    if (!path.hasToProj())
      if (rc[path.to] === undefined) rc[path.to] = [path];
      else rc[path.to].push(path);
  }
  return rc;
};
//******************************************************************************
...
FGraphInfo.prototype.addTreePath = function (sourceBox, targetBox) {
  ...
  var nextBox = this.addGenBox(
    box.type === "forkFlow" ? "nodeFlow" : "forkFlow",
  );
...
FGraphInfo.prototype.treeGenerator = function (n) {
  ...
  for (var i = 1; i < n; i++) this.addTreePath();
};
  ...
  while (i < startBoxes.length) {
    ...
    _.each(box.paths, function (path) {
      ...
      if (otherBox.level === undefined) {
        ...
        if (box.level + 1 >= lInd.length) lInd.push(0);
...
FGraphInfo.prototype.normalizeLevels = function () {
  ...
  for (var i = 0; i < this.levels.length; i++) {
    ...
    for (var j = 1; j < level.length; j++) {
      ...
      box1.center[0] =
        box0.center[0] + (box0.size[0] + box1.size[0]) / 2 + this.spacing;
...
FGraphInfo.prototype.layout = function () {
  ...
  _.each(this.boxes, function (box) {
    if (levNum < box.level) levNum = box.level;
  });
  for (var i = 0; i <= levNum; i++) this.levels.push([]);
  var self = this;
  _.each(this.boxes, function (box) {
