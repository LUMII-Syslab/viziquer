...
ConnArea.prototype.initTestRect = function () {
  ...
  this.testRect[1 - d] = this.box.offValue(
    this.testRect[(this.side > 1 ? 3 : 1) - d],
    this.side,
  );
  this.testRect[3 - d] = this.testRect[1 - d];
  this.segments = [];

  if (this.disconnPaths.length > 3) var iii = 0;
  ...
  _.each(this.disconnPaths, function (path) {
    if (path.isSelfloop()) {
      ...
      segm = self.box.offSegm(
        path.from === self.box.id ? path.segm(1) : path.segm(path.n - 1),
        self.side,
      );
      self.initDisconnSegm(segm);
    }
...
ConnArea.prototype.createIntervals = function () {
  ...
  for (j = 0; j < this.segments.length; j++) {
    ...
    if (!segm.disconnected)
      if (segm.mark !== 0) this.setCoordInd(segm.lev);
      else {
  ...
  //    this.indCoord.sort();
  this.indCoord.sort(function (a, b) {
    return a < b ? -1 : a > b ? 1 : 0;
  });

  for (j = 0; j < this.coors; j++) this.coordInd[this.indCoord[j]] = j;
...
ConnArea.prototype.processForward = function (segm) {
  ...
  while (i < this.coors - 1) {
    for (
      j = i + 1;
      j < this.coors - 1 &&
      this.indHPenalty[j] === 0 &&
      this.indVPenalty[j] === 0;
      j++
    );
...
ConnArea.prototype.processBackward = function (segm) {
  ...
  while (i > 0) {
    for (
      j = i - 1;
      j > 0 && this.indHPenalty[j] === 0 && this.indVPenalty[j] === 0;
      j--
    );
...
ConnArea.prototype.processDisconnected = function (segm) {
  ...
  if (segm.forward(this.box.id)) return this.processForward(segm);
  ...
  _.each(this.segments, function (segm) {
    if (segm.disconnected) {
      ...
      if (pair === undefined || pair[0] >= pair[1])
        warning(
          "ConnArea.prototype.processSegms wrong pair " + pair.toString(),
        );
      else {
        pairV = self.getValueOfPair(pair);
        if (self.pairs[pairV] === undefined) self.pairs[pairV] = [];
    ...
    } else self.indVPenalty[self.coordInd[segm.lev]] += segm.mark;
...
ConnArea.prototype.connectSegms = function () {
  ...
  _.each(this.pairs, function (segmList, pairV) {
    ...
    str +=
      ...
      pair.toString() +
      "]; forw:" +
      segmList[0].forward(self.box.id) +
      "; levs:";
    _.each(segmList, function (segm) {
      path = segm.owner;
      //            str += "\n" + path.toString() + "\n";
      isLast =
        (path.from === path.to && segm.ind > 1) ||
        (path.from !== path.to && self.box.id === path.to);
      str += "; " + segm.toString() + ": " + v + ";" + "; last " + isLast;
      m = self.box.center[segm.dir];
      if (segm.dir < 0 || segm.dir > 1 || m === undefined)
        warning(
          "ConnArea.prototype.connectSegms wrong added coordinate " +
            m +
            "; segm.dir " +
            segm.dir,
        );
...
ConnArea.prototype.simplifyPaths = function () {
  ...
  _.each(this.connPaths, function (path) {
    if (path.isSelfloop())
      if (path.fromSide() === path.toSide()) selfloops.push(path);
    ...
    else if (path.n === 2) straights.push(path);
  ...
  segments.sort(function (a, b) {
    ...
    else if (af < b.forward(id)) return 1;
    else return -1;
  });
  var d = this.side & 1;
  ...
  var k = 0;
  while (k < segments.length && segments[k].forward(id)) k++;
