import ConnArea from "./line_routing_connArea.js";
...
BoxInfo.prototype.hasSideCrossings = function (paths, side) {
  ...
  _.each(paths, function (path) {
    ...
    if (path.n > 2)
      segments[1 - dir].push(
        path.from === id ? path.segm(2) : path.segm(path.n - 2),
      );
    if (path.n > 3)
      segments[dir].push(
        path.from === id ? path.segm(3) : path.segm(path.n - 3),
      );
  });
  ...
  if (n === 2) {
    ...
    lev =
      this.id === path.from
        ...
        : [
            this.offValue(levs[n], side),
            levs[n - 1],
            this.offValue(levs[n - 2], side),
          ];
  }
  if (n === 3) {
    if (this.id === path.from)
      lev = [
        this.offValue(levs[0], side),
        levs[1],
        this.offValue(levs[2], side),
        levs[3],
      ];
    else
      lev = [
  ...
  //     0 1 2 4 8 9 10
    ...
    if (this.id === path.from)
      ...
      lev = [
        ...
        this.offValue(levs[n - 2], side),
        levs[n - 3],
        this.offValue(levs[n - 4], side),
      ];
    type =
      lev[1] < lev[3] ? (lev[2] < lev[4] ? 8 : 10) : lev[2] < lev[4] ? 2 : 0;
  }

  return { owner: path, type: type, lev: lev };
};
BoxInfo.prototype.reduceSideCrossings = function (paths, side) {
  if (paths.length < 2 || !this.hasSideCrossings(paths, side)) return;
  ...
  var turns = [];
  _.each(paths, function (path) {
    turns.push(self.getTurn(path, side));
  });
  ...
  turns.sort(function (a, b) {
    return a.lev[1] < b.lev[1] ? -1 : a.lev[1] > b.lev[1] ? 1 : 0;
  });
  coords[1] = _.map(turns, function (turn) {
    return turn.lev[1];
  });

  // order 2 segm of turn list
  turns.sort(function (a, b) {
    return self.Compare2Turns(a, b);
  });
  var midPathIds = _.map(turns, function (turn) {
    return turn.owner.id;
  });
  coords[2] = _.map(turns, function (turn) {
    return turn.lev[2];
  });

  // order 3 segm of turn list
  turns.sort(function (a, b) {
    return self.Compare3Turns(a, b);
  });
  var farPathIds = _.map(turns, function (turn) {
    return turn.owner.id;
  });
  coords[3] = _.map(turns, function (turn) {
    return turn.lev[3];
  });
  ...
  _.each(coords, function (coord, i) {
    _.each(turns, function (turn, j) {
      path = turn.owner;
      if ((i & 1) === 0) {
        if (path.from === id) path.lev[i] = self.onValue(coord[j], side);
        else path.lev[path.n - i] = self.onValue(coord[j], side);
      } else if (path.from === id) path.lev[i] = coord[j];
      else path.lev[path.n - i] = coord[j];
    });
  });
  _.each(paths, function (path) {
    path.clipOnBoxes();
...
BoxInfo.prototype.Compare2Turns = function (a, b) {
  ...
  // type === 0, 1, 2, 4
    ...
    if (a.lev[1] === b.lev[1]) return 0;
    else if (a.lev[1] < b.lev[1]) return -1;
    else return 1;
  else if (a.type <= 4)
    if (a.lev[2] === b.lev[2]) return 0;
    else if (a.lev[2] < b.lev[2]) return -1;
    else return 1;
  //   |_____  |  ____|
  //  _____  | | |  ____
  //    _  | | | | |  _
  ...
  //
  // type === 8, 9, 10
  else if (a.lev[2] === b.lev[2]) return 0;
  else if (a.lev[2] < b.lev[2]) return 1;
  else return -1;
};
BoxInfo.prototype.Compare3Turns = function (a, b) {
  if (a.type < b.type) return -1;
    ...
    if (a.type === 2)
      if (a.lev[3] === b.lev[3]) return 0;
      else if (a.lev[3] < b.lev[3]) return -1;
      else return 1;
    else if (a.type === 0)
      if (a.lev[3] === b.lev[3]) return 0;
      else if (a.lev[3] < b.lev[3]) return 1;
  ...
  // type === 8
  else if (a.lev[3] === b.lev[3]) return 0;
  else if (a.lev[3] < b.lev[3]) return -1;
  else return 1;
};
...
BoxInfo.prototype.createConnArea = function (side, disconnPaths, connPaths) {
  ...
  for (let i = 0; i < disconnPaths.length; i++) {
    path = disconnPaths[i];
    var segm = this.offSegm(
      path.from === this.id ? path.segm(1) : path.segm(path.n - 1),
      side,
    );
    rc.disconnectedSegm.push(segm);
    if (testBox[3 - d] < segm.lev) testBox[3 - d] = segm.lev;
  }
  rc.testBox = testBox;
...
BoxInfo.prototype.processDisconnectPaths = function () {
  ...
  _.each(this.paths, function (path) {
    if (!path.isSelfloop())
      paths[path.hasProj(self.id) ? conn : disconn][path.side(self.id)].push(
        path,
      );
    else {
...
BoxInfo.prototype.processDisconnectedPathsNew = function (pathList) {
  ...
  for (let j = 0; j < pathList.length; j++) {
    ...
    if (boxPathSide[side] === undefined) boxPathSide[side] = [[path], []];
    else boxPathSide[side][0].push(path);
  }
  ...
  for (let side in boxPathSide)
    this.processSideDisconnectedPaths(
      side,
      boxPathSide[side][0],
      boxPathSide[side][1],
    );
...
BoxInfo.prototype.processDisconnectedPaths = function () {
  ...
  for (let i = 0; i < this.paths.length; i++) {
    ...
    if (this.id === path.from && path.hasToProj())
      if (nodes[path.from] === undefined) nodes[path.from] = [path];
      else nodes[path.from].push(path);
    if (this.id === path.to && path.hasFromProj())
      if (nodes[path.to] === undefined) nodes[path.to] = [path];
      else nodes[path.to].push(path);
  }
...
BoxInfo.prototype.onResizeBox = function () {
  ...
  if (!noChanges && rect[2] > rect[0] && rect[3] > rect[1]) {
    ...
    var prop = [
      (rect[2] - rect[0]) / (initRect[2] - initRect[0]),
      (rect[3] - rect[1]) / (initRect[3] - initRect[1]),
    ];
    _.each(this.paths, function (path) {
      if (path.from === id) {
        d = path.dir;
        ...
        path.lev[1] =
          (path.lev[1] - initRect[1 - d]) * prop[1 - d] + rect[1 - d];
      }
      if (path.to === id) {
        i = path.n;
        d = (path.dir + i) & 1;
        ...
        path.lev[i - 1] =
          (path.lev[i - 1] - initRect[1 - d]) * prop[1 - d] + rect[1 - d];
      }
    });
  }
};
BoxInfo.prototype.simplifyBoxPaths = function () {
  ...
  _.each(this.paths, function (path) {
    ...
    if (path.isSelfloop())
      ...
      } else paths[s].push(path);
    else if (id === path.from) paths[s].push(path);
    else paths[path.toSide()].push(path);
  });
  for (s = 0; s < 4; s++)
    if (paths[s].length > 0) {
...
BoxInfo.prototype.onValue = function (v, side) {
  var d = 1 - (side & 1);
  if ((side & 2) > 0) return v + this.max(d);
  else return this.min(d) - v;
};
...
BoxInfo.prototype.offSegm = function (segm, side) {
  var d = 1 - (side & 1);
  if (d === segm.dir) segm.lev = this.offValue(segm.lev, side);
  else {
    if ((side & 2) > 0) {
      segm.min -= this.max(d);
      segm.max -= this.max(d);
...
BoxInfo.prototype.isFork = function () {
  ...
  if (
    type_name === "HorizontalLine" ||
    type_name === "VerticalLine" ||
    type_name === "HorizontalFork" ||
    type_name === "VerticalFork"
  ) {
    return true;
  }
};
