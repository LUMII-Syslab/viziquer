...
var Palette = function (editor, palette_obj) {
  ...
  var paletteStage = new Konva.Stage({
    container: editor.paletteContainerName,
    width: palette.width,
...
var PaletteButton = function (palette) {
  ...
  paletteButton.build_button_container = function (
    palette_layer,
    palette_button,
  ) {
    var button_container = new Konva.Group({
      x: palette_button.x,
      y: palette_button.y,
    });
  ...
  paletteButton.build_button_rect = function (
    button_container,
    palette_button,
  ) {
    ...
    (paletteButton.addLine = function (button_container, palette_button) {
      ...
      if (start_shape_style) {
        if (start_shape_style.radius)
          start_shape_style.radius = compute_palette_line_end_shape_radius(
            start_shape_style.radius,
          );

        if (start_shape_style.width)
          start_shape_style.width = compute_palette_line_end_shape_radius(
            start_shape_style.width,
          );

        if (start_shape_style.height)
          start_shape_style.height = compute_palette_line_end_shape_radius(
            start_shape_style.height,
          );
      }
      ...
      if (end_shape_style) {
        ...
        end_shape_style.radius = compute_palette_line_end_shape_radius(
          end_shape_style.radius,
        );
      ...
      //This is a hack
      var image_src = elem_style.imageSrc;
      //		    console.log("LINK IMG1",image_src);
      if (image_src) {
        delete elem_style.imageSrc;
        var x = 5;
        var y = 7;
        ...
        var box = editor.elements.createShape("Rectangle");
        box.render(button_container, properties);
  ...
  paletteButton.getBackgroundRect = function () {
    ...
    return this.palette.editor.findChild(button_container, "ButtonRect");
  };
  ...
  function addTooltip(y, tooltipText, stage) {
    ...
    let absoluteTop_y =
      document.getElementById("ajoo_scene").getBoundingClientRect().top -
      document.getElementById("ajoo_scene").children[0].getBoundingClientRect()
        .top;

    let absoluteTop_x =
      document.getElementById("ajoo_scene").getBoundingClientRect().left -
      document.getElementById("ajoo_scene").children[0].getBoundingClientRect()
        .left;
  ...
  paletteButton.setActiveStyle = function () {
    ...
    // console.log(palette_button_rect)
    // console.log("Y = ", palette_button_rect.parent.attrs.y)
    ...
    // const AjooEditorVueComponent = Vue.$children[0].$children[0].$children[3]
    // let tooltipText = this.data.elementTypeId
    // for(let i=0; i < AjooEditorVueComponent.elementTypes.length; i++ ){
    // 	let elemType = AjooEditorVueComponent.elementTypes[i]
    // 	if(elemType.id === this.data.elementTypeId){
    // 		tooltipText += " / " + elemType.name
    // 	}
    // }
    ...
    //console.log(mainjs)
    //console.log(paletteButton.palette.editor)
    var fill = "#eee";
    palette_button_rect.fill(fill);

    paletteButton.refresh();

    // addTooltip(palette_button_rect.parent.attrs.y + palette_button_rect.attrs.height/2, tooltipText, this.palette.editor.stage)
  };

  paletteButton.setDefaultStyle = function (is_no_refresh) {
    //console.log("setdefaultstyle")
    var palette_button_rect = paletteButton.getBackgroundRect();
    var fill = "#fff";
    palette_button_rect.fill(fill);

    if (is_no_refresh) return;
...
PaletteButton.prototype = {
  add: function (palette_button) {
    ...
    var button_container = paletteButton.build_button_container(
      palette_layer,
      palette_button,
    );
  ...
  pressPaletteButton: function () {
    ...
    palette.state = { pressed: true, pressedButton: paletteButton };
...
function PaletteButtonHandlers(paletteButton) {
  ...
  var button_container = paletteButton.presentation;
  var palette_button_overlay = palette.editor.findChild(
    button_container,
    "Overlay",
  );
  ...
  palette_button_overlay.on("mouseover", function (e) {
    palette.editor.setCursorStyle("pointer");
    paletteButton.setActiveStyle();
  ...
  palette_button_overlay.on("mouseleave", function (e) {
    ...
    if (!pressed_button) {
      palette.editor.setCursorStyle("default");
