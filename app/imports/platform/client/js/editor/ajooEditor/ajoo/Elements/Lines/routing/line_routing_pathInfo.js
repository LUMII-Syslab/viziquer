...
PathInfo.prototype.fromSide = function () {
  if (this.hasFromProj())
    if (this.lev[0] < this.lev[2]) return 3 - this.dir;
    else return 1 - this.dir;
  else if (this.lev[1] < this.fromCenter[1 - this.dir]) return this.dir;
  else return 2 + this.dir;
};
PathInfo.prototype.toSide = function () {
  var dir = (this.n + this.dir) & 1;
  if (this.hasToProj())
    if (this.lev[this.n] < this.lev[this.n - 2]) return 3 - dir;
    else return 1 - dir;
  else if (this.lev[this.n - 1] < this.toCenter[1 - dir]) return dir;
  else return 2 + dir;
};
...
PathInfo.prototype.fromInsideTo = function () {
  return (
    this.toRect[0] < this.fromRect[0] &&
    this.toRect[2] > this.fromRect[2] &&
    this.toRect[1] < this.fromRect[1] &&
    this.toRect[3] > this.fromRect[3]
  );
};
PathInfo.prototype.dummyPath = function (f, t) {
  ...
  if (f[0] <= t[2] && t[0] <= f[2]) {
    ...
    else v = [f[3], (Math.max(f[0], t[0]) + Math.min(f[2], t[2])) / 2, t[1]];
    str += "::1:: v " + v.toString();
  } else if (f[1] <= t[3] && t[1] <= f[3]) {
    this.dir = 0;
    if (f[0] >= t[2])
      v = [f[0], (Math.max(f[1], t[1]) + Math.min(f[3], t[3])) / 2, t[2]];
    else v = [f[2], (Math.max(f[1], t[1]) + Math.min(f[3], t[3])) / 2, t[0]];
    str += "::2:: v " + v.toString();
  } else {
    this.dir = 0;
    if (f[2] < t[0])
      if (f[3] < t[1]) v = [f[2], this.fromCenter[1], this.toCenter[0], t[1]];
      else v = [f[2], this.fromCenter[1], this.toCenter[0], t[3]];
    else if (f[3] < t[1])
      v = [f[0], this.fromCenter[1], this.toCenter[0], t[1]];
    else v = [f[0], this.fromCenter[1], this.toCenter[0], t[3]];
    str += "::3:: v " + v.toString();
  }
  this.lev = v;
  this.n = this.lev.length - 1;
...
PathInfo.prototype.dummyDeltaPath = function (delta2) {
  ...
  if (max[0] > delta2) {
    ...
    if (max[1] < 4)
      if (max[1] < 2)
        if (max[1] < 1)
          ...
          else v = [f[1], f[0] + delta2, f[1] - delta2, t[0] - delta2, t[1]];
        // 1
        else if (f[1] < t[1])
          v = [f[0], t[1] - delta2, t[0] - delta2, t[1] + delta2, t[0]];
        else v = [f[0], f[1] + delta2, f[0] - delta2, t[1] - delta2, t[0]];
      else if (max[1] < 3)
        if (f[0] < t[0])
          // 2
          v = [f[3], t[0] - delta2, t[3] + delta2, t[0] + delta2, t[3]];
        else v = [f[3], f[0] + delta2, f[3] + delta2, t[0] - delta2, t[3]];
      // 3
      else if (f[1] < t[1])
        v = [f[0], f[3] - delta2, f[0] - delta2, f[3] + delta2, t[0]];
      else v = [f[0], t[3] + delta2, t[0] - delta2, t[3] - delta2, t[0]];
    else if (max[1] < 6)
      if (max[1] < 5)
        if (f[2] > t[2])
          // 4
          v = [f[1], t[2] + delta2, t[1] - delta2, t[2] - delta2, t[1]];
        else v = [f[1], f[2] + delta2, f[1] - delta2, f[2] - delta2, t[1]];
      // 5
      else if (f[1] < t[1])
        v = [f[2], t[1] - delta2, t[2] + delta2, t[1] + delta2, t[2]];
      else v = [f[2], f[1] - delta2, f[2] + delta2, f[1] + delta2, t[2]];
    else if (max[1] < 3)
      if (f[0] < t[0])
        // 6
        v = [f[1], t[0] - delta2, t[1] - delta2, t[0] + delta2, t[1]];
    ...
    else v = [f[2], f[3] + delta2, f[2] + delta2, f[3] - delta2, t[3]];
...
PathInfo.prototype.setDummySinglePath = function () {
  ...
  if (
    this.n < 2 ||
    !this.hasFromProj(this.from) ||
    !this.hasFromProj(this.to)
  ) {
...
PathInfo.prototype.setDummySelfloop = function () {
  ...
  if (corner === 0)
    this.lev = [
      this.fromRect[0] + delta,
      this.fromRect[1] - delta,
      this.fromRect[0] - delta,
      this.fromRect[1] + delta,
    ];
  else if (corner === 1)
    this.lev = [
      this.fromRect[0] + delta,
      this.fromRect[3] + delta,
      this.fromRect[0] - delta,
      this.fromRect[3] - delta,
    ];
  else if (corner === 2)
    this.lev = [
      this.fromRect[2] - delta,
      this.fromRect[1] - delta,
      this.fromRect[2] + delta,
      this.fromRect[1] + delta,
    ];
...
PathInfo.prototype.setDummyPath = function () {
  if (this.isSelfloop()) this.setDummySelfloop();
...
PathInfo.prototype.clipOnBoxes = function () {
  ...
  var new_start_point_obj = start_segment_svg.getIntersectionWithElement(
    this.fromObject,
    [point1.c[0], point1.c[1]],
  );
  ...
  var new_end_point_obj = end_segment_svg.getIntersectionWithElement(
    this.toObject,
    [point0.c[0], point0.c[1]],
  );
...
PathInfo.prototype.buildEndSegmentForClipping = function (point0, point1) {
  ...
  //vertical
  else line_points = [point0.c[0], -inf, point1.c[0], inf];
...
PathInfo.prototype.getSegmIntersectWithToBox = function (ind) {
  ...
  for (var i = ind; rc < 0 && i < this.n; i++)
    if (this.segm(i).intersectRect(this.toRect)) rc = i;
...
PathInfo.prototype.getSegmIntersectWithFromBox = function (ind) {
  ...
  for (var i = ind; rc < 0 && i > 0; i--)
    if (this.segm(i).intersectRect(this.fromRect)) rc = i;
...
PathInfo.prototype.getFirstPointOutsideFromBox = function (ind) {
  ...
  var rc = -1;
  for (var i = ind; rc < 0 && i < this.n; i++)
    if (!this.point(i).inside(this.fromRect)) rc = i;
  //    printText("\t getSegmIntersectWithFromBox: " + rc + "; ind" + ind + "; rect:" + this.fromRect.toString());
  return rc;
};
PathInfo.prototype.getFirstPointOutsideToBox = function (ind) {
  ...
  var rc = -1;
  for (var i = ind; rc < 0 && i >= 0; i--)
    if (!this.point(i).inside(this.toRect)) rc = i;
  //    printText("\t getSegmIntersectWithFromBox: " + rc + "; ind" + ind + "; rect:" + this.fromRect.toString());
  return rc;
};
...
PathInfo.prototype.hasProj = function (boxId) {
  var rc = true;
  if (this.from === boxId) rc &= this.hasFromProj();
  if (this.to === boxId) rc &= this.hasToProj();
  return rc;
};
...
PathInfo.prototype.cutPoints = function (cutList) {
  ...
  if (cutList.length === 2 && cutList[0] === 0 && cutList[1] === this.n) return;
  ...
  this.ind = m;
  var newLev = [];
  if ((cutList[0] & 1) === 1) this.dir = 1 - this.dir;
  for (let i = 0; i < k; i += 2)
    for (let j = cutList[i]; j <= cutList[i + 1]; j++) newLev.push(this.lev[j]);
  this.lev = newLev;
  this.n = this.lev.length - 1;
};
PathInfo.prototype.simplifyFrom = function () {
  if (this.testPath("input simplifyFrom", "", false, false)) return;
  var ps = this.point(0);
  var pf = this.point(this.n - 1);
  if (
    this.point(0).inside(this.toRect) ||
    ...
    if (j < this.n - 1) {
      if (j === -1) {
        if (!this.hasToProj()) this.addToBend();
        j = this.n;
      } else {
    ...
    if (i === -1) {
      if (!this.hasFromProj()) this.addFromBend();
    } else if (i > 1) {
  ...
  this.n = this.lev.length - 1;
  this.clipOnBoxes();
  if (this.testPath("output simplifyFrom", "", false, true)) return;
};
PathInfo.prototype.simplifyTo = function () {
  ...
  this.simplifyFrom();
  this.reverse();
  if (this.testPath("output simplifyTo", "", false, true)) return;
};
PathInfo.prototype.smoothConnection = function (boxId) {
  ...
  if (
    this.segm(v).projRect(otherRect) &&
    vs[0] < otherRect[2 + dir] &&
    otherRect[dir] < vs[1]
  ) {
    if (vs[0] < otherRect[dir]) vs[1] = otherRect[dir];
    else vs[0] = otherRect[2 + dir];
  }

  var test;
  if (side === 0) test = [vs[0], this.lev[v], vs[1], rect[1]];
  else if (side === 1) test = [this.lev[v], vs[0], rect[0], vs[1]];
  else if (side === 2) test = [vs[0], rect[1], vs[1], this.lev[v]];
  else if (side === 3) test = [rect[2], vs[0], this.lev[v], vs[1]];
  else {
    // warning("PathInfo.prototype.smoothConnectionNew wrong side " + side);
    return;
  }
  var testSegm = new SegmInfo([
    1 - dir,
    test[dir],
    this.lev[v],
    test[2 + dir],
  ...
  var sign =
    ...
    ((this.from === boxId && this.lev[1] < this.lev[3]) ||
      (this.to === boxId && this.lev[this.n - 1] < this.lev[this.n - 3]))
      ? -1
      : 1;
      ...
      for (var i = 1; i < path.n; i++) {
        ...
        if (segm.intersectRect(test)) {
          ...
          if (dir === segm.dir) {
            ...
            if (levMap[v] === undefined) levMap[v] = 0;
            levMap[v] += sign;
  ...
  if (levList.length < 2) levList.push(levList[0]);
...
PathInfo.prototype.simplifySelfloop = function () {
  ...
  if (f < t && (f > 1 || t < this.n - 1)) this.cutPoints([f - 1, t + 2]);
};
PathInfo.prototype.simplifyPath = function () {
  if (
    this.point(0).inside(this.toRect) ||
    this.point(this.n - 1).inside(this.fromRect)
  ) {
    this.setDummyPath();
  ...
  if (sf >= 0 && st >= 0) {
    if (
      ...
      (sf === st &&
        (rOR ||
          this.toCenter[dir] < this.fromCenter[dir] !==
...
PathInfo.prototype.simplify = function () {
  ...
  if (this.isSelfloop()) this.simplifySelfloop();
  else this.simplifyPath();
...
PathInfo.prototype.removeStep = function (k) {
  ...
  for (var i = k; i <= this.n; i++) this.lev[i - 2] = this.lev[i];
  this.lev.pop();
  this.lev.pop();
  this.n = this.lev.length - 1;
};
PathInfo.prototype.removeGaps = function (ind) {
  ...
  var k = ind;
  var str =
    "removeGaps k " + k + "; n " + this.n + "; lev: " + this.lev.toString();
  if (this.isSelfloop() && this.n < 6) {
    if (this.n === 4 && k === 2) {
      var d = this.dir;
      if (
        Math.abs(this.lev[k] - this.toCenter[d]) <
        this.toCenter[d] - this.toRect[d] + this.owner.limPathGap()
      )
        if (this.lev[k] < this.toCenter[d])
          this.lev[k] = this.toRect[d] - this.owner.limPathGap();
        else this.lev[k] = this.toRect[2 + d] + this.owner.limPathGap();
  ...
  if (k < this.n - 2) {
    var gap = Math.abs(this.lev[k + 2] - this.lev[k]);
    str += "; gap " + gap;
    if (gap < this.owner.limPathGap()) this.removeStep(k + 2);
    str += "; newlev: " + this.lev.toString();
  }
  // printText(str);
...
PathInfo.prototype.avoidPathCrossings = function () {
  ...
  if (sf !== -1 && st !== -1)
    if (sf > st) this.setDummyPath();
    else {
      if (sf > 1 || st < this.n - 2) this.cutPoints([sf - 1, st + 1]);
    }
  else {
    if (sf === -1) {
      if (st > 0 && st < this.n - 1) this.cutPoints([0, st + 1]);
      if (!this.hasFromProj()) this.addFromBend();
    }
    if (st === -1) {
      if (sf > 1) this.cutPoints([sf - 1, this.n]);
      if (!this.hasToProj()) this.addToBend();
    }
  }
  this.lev[0] = this.fromCenter[this.dir];
