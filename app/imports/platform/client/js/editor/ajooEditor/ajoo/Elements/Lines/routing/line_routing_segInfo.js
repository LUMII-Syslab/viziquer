...
var SegmInfo = function (info) {
  ...
  this.increase =
    this.owner.lev[this.ind - 1] < this.owner.lev[this.ind + 1] ? 0 : 1;
  this.ntype = "s";
};
SegmInfo.prototype.forward = function (boxId) {
  if (this.ind < 1 || this.ind >= this.owner.n)
    warning(
      "SegmInfo.prototype.forvard wrong segment index " +
        this.ind +
        "; " +
        this.owner.toString(),
    );
  ...
  else rc = (boxId === this.owner.from) === (this.increase === 1);
  return rc;
};
...
SegmInfo.prototype.level = function () {
  if (this.mark === 0) return this.lev;
  if (this.mark === 1) return this.min;
  return this.max;
};
SegmInfo.prototype.compareConn = function (segm) {
  if (this.level() < segm.level()) return -1;
  else if (this.level() > segm.level()) return 1;
  else if (this.mark === segm.mark) return 0;
  else if (this.mark > segm.mark) return -1;
  else return 1;
};
...
SegmInfo.prototype.intersectSegm = function (s) {
  ...
  else rc = this.projSegm(s) && s.projSegm(this);
  return rc;
};
SegmInfo.prototype.projSegm = function (s) {
  ...
  if (this.dir === s.dir)
    warning("SegmInfo.prototype.projSegm: segments has same direction");
  else rc = this.lev <= s.max && s.min <= this.lev;
  return rc;
};
