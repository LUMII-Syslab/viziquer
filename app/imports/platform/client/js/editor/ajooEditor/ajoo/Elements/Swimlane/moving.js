...
MovingSwimlane.prototype = {
  startDragging: function () {
    ...
    _.each(line_parent.children, function (tmp_line) {
      if (tmp_line.index === index - 1) prev_line = tmp_line;
      else if (tmp_line.index === index + 1) next_line = tmp_line;
    });
  ...
  compute_line_position: function (prev, next, mouse_pos) {
    if (!mouse_pos) mouse_pos = 0;

    var new_pos;

    //if mouse is between prev and next line
    if (prev <= mouse_pos && mouse_pos <= next) new_pos = mouse_pos;
    //if mouse is before prev line
    else if (prev > mouse_pos) new_pos = prev;
    //if mouse is after next line
    else if (next < mouse_pos) new_pos = next;
