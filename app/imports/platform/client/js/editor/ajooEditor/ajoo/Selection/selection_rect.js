...
var SelectionRect = function (editor) {
  ...
  selectionRect.getRectPoints = function () {
    ...
    if (!rectangle) console.error("Error in: selectionRect.getRectPoints");
  ...
  selectionRect.detectIntersection = function (selection_points) {
    ...
    var selection_elems = _.filter(
      selectionRect.editor.getElements(),
      function (element) {
        //selecting boxes
        if (element.type === "Box") {
          var shape_size = element.getSize();
          var shape_points = order_shape_points(
            shape_size.x,
            shape_size.y,
          ...
          if (
            selection_points["x1"] < shape_points["x1"] &&
            selection_points["x2"] > shape_points["x1"] &&
            selection_points["x1"] < shape_points["x2"] &&
            selection_points["x2"] > shape_points["x2"] &&
            selection_points["y1"] < shape_points["y1"] &&
            selection_points["y2"] > shape_points["y1"] &&
            selection_points["y1"] < shape_points["y2"] &&
            selection_points["y2"] > shape_points["y2"]
          ) {
            return true;
          }
        }
        ...
        //selecting lines
          ...
          if (points) {
            ...
            var res = _.find(points, function (point, j) {
              if (j % 2 === 0) {
                if (
                  point < selection_points["x1"] ||
                  selection_points["x2"] < point
                )
                  return true;
              } else {
                if (
                  point < selection_points["y1"] ||
                  selection_points["y2"] < point
                )
                  return true;
              }
            });
