...
var SVGObject = function (shape_path) {
  ...
  _.each(pathes_in, function (path) {
    ...
    var path_node = document.createElementNS(
...
LineSVGObject.prototype = {
  ...
  getIntersectionWithElement: function (element, line_point) {
    ...
    if (line_svg_obj && shape_svg_obj) {
      var inter_points = line_svg.computeIntersection(
        shape_svg_obj,
        line_svg_obj,
      );
      return line_svg.get_closest_point_from_intersections(
        line_point,
        inter_points,
      );
    } else {
  ...
  get_closest_point_from_intersections: function (
    segment_start_point,
    inter_points,
  ) {
    var line_svg = this;
    ...
    _.each(inter_points, function (inter_point) {
      ...
      var distance = line_svg.compute_distance_between_points(
        segment_start_point,
        tmp_point,
      );
  ...
  build_line_path: function (points) {
    ...
    for (var i = 0; i < points.length; i = i + 2) {
      if (i === 0) path = "M" + points[i] + "," + points[i + 1];
      else path = path + " L" + points[i] + "," + points[i + 1];
    }
