...
var LinkCompartments = function (element, comparts_in) {
  ...
  compartments.textsGroup = compartments.editor.findChild(
  ...
  compartments.functions = {
    north: compartments.vertical_right_end,
...
LinkCompartments.prototype = {
  create: function (comparts_in) {
    ...
    _.each(comparts_in, function (compart_in) {
      if (
        (compart_in &&
          (compart_in.value === "" || _.isUndefined(compart_in.value))) ||
  ...
  computeGroupPosition: function (placement) {
    ...
    var placements = {
      ...
      "middle-left": function () {
        ...
        var position = compartments.buildPositionPath(
          index,
          index + 1,
          index + 2,
          index + 3,
        ...
        return compartments.labelLayout(
          position,
          width,
          height,
          size,
  ...
  compute_start_left_position: function (width, height, size) {
    var compartments = this;
    return compartments.compute_line_text_position(
      width,
      height,
      size,
      "start",
  ...
  compute_line_text_position: function (
    width,
    height,
    size,
    direction,
    ...
    if (points[x1] === points[x2] || points[y1] === points[y2]) {
      ...
      var rotation = compartments.compute_rotation(
        points,
        x1,
        y1,
        x2,
  ...
  buildPositionPath: function (x1, y1, x2, y2) {
    ...
    if (points[x1] === points[x2]) {
      ...
      if (points[y1] < points[y2]) position += "-south-";
      else position += "-north-";
    }
    ...
    //if horizontal
      ...
      if (points[x1] < points[x2]) position += "-east-";
      else position += "-west-";
    }
  ...
  horizontal_left_start: function (x, y, width, height, size) {
    ...
    if (size.width === 0 && size.height === 0) padding.height = 4;
  ...
  horizontal_right_start: function (x, y, width, height, size) {
    var padding = { width: 3, height: 0 };
    if (size.width === 0 && size.height === 0) padding.height = 3;

    return { x: x + padding.width, y: y + padding.height + size.width };
  },
  ...
  vertical_left_start: function (x, y, width, height, size) {
    ...
    if (size.width === 0 && size.height === 0) padding.width = 4;

    return { x: x + padding.width + size.width, y: y + padding.height };
  },

  vertical_right_start: function (x, y, width, height, size) {
    var padding = { width: 2, height: 3 };
    if (size.width === 0 && size.height === 0) padding.width = 5;

    return {
      x: x - (width + padding.width + size.width),
      y: y + padding.height,
    };
  },
  ...
  get_end_shape_size: function (name) {
    ...
    var end_shape_obj;
    if (name === "Start") end_shape_obj = line.startElement;
    else end_shape_obj = line.endElement;

    var shapes = end_shape_obj.presentation;
    if (!shapes) return { width: 0, height: 0 };
    else {
      var shape = shapes[0];
  ...
  get_shape_size: function (shape) {
    if (shape.name === "Arrow") {
      ...
      shape.className === "RegularPolygon" ||
      shape.className === "Circle"
    ) {
      var radius = shape.radius();
      if (!radius) return { width: 0, height: 0 };
      else return { width: radius * 2, height: radius * 2 };
    }
  },

  compute_rotation: function (
    points,
    x1_index,
    y1_index,
    x2_index,
  ...
  move_center: function (
    ...
    if (move_func) return move_center_func[move_func]();
    else {
      return { x: x, y: y };
    }
  },
  ...
  removeOne: function (compart_id, is_refresh_not_needed) {
    ...
    delete comparts_list[compart_id];
    compartments.compartments = _.filter(
      compartments.compartments,
      function (compart) {
        if (compart._id !== compart_id) return true;
      },
    );
  ...
  computeMiddlePoint: function () {
    ...
    for (var i = 0; i < points.length - 2; i = i + 2) {
      ...
      else line_length += Math.abs(points[i] - points[i + 2]);
    }
    ...
    for (var i = 0; i < points.length - 2; i = i + 2) {
      ...
      else tmp_distance = points[i] - points[i + 2];
      ...
      if (tmp_len + tmp_distance >= middle_point) {
        ...
        if (points[i] === points[i + 2]) {
          if (points[i + 1] < points[i + 3])
            point = {
              x: points[i],
              y: points[i + 1] + (middle_point - tmp_len),
            };
          else
            point = {
              x: points[i],
        ...
        //if horizontal
          ...
          if (points[i] < points[i + 2])
            point = {
              x: points[i] + (middle_point - tmp_len),
              y: points[i + 1],
            };
          else
            point = {
  ...
  labelLayout: function (position, width, height, size, x1, y1, name) {
    ...
    if (!name) {
      name =
        compartments.editor.lineSettings.compartmentLayout ||
        "lineOrientedFlow";
    }
    ...
    var layouts = {
      lineOrientedFlow: {
        ...
        "horizontal-east-left": function () {
          return compartments.horizontal_right_start(
            x1,
            y1,
            width,
            height,
        ...
        "horizontal-east-right": function () {
          return compartments.horizontal_left_start(
            x1,
            y1,
            width,
            height,
        ...
        "horizontal-west-middle-left": function () {
          return compartments.horizontal_right_middle(
            x1,
            y1,
            width,
            height,
        ...
        "horizontal-west-middle-right": function () {
          return compartments.horizontal_left_middle(
            x1,
            y1,
            width,
            height,
        ...
        "vertical-north-middle-right": function () {
          return compartments.vertical_right_middle(
            x1,
            y1,
            width,
            height,
