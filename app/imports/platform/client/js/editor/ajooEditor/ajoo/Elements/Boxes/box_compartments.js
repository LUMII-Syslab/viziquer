...
BoxCompartments.prototype = {
  create: function (comparts_in) {
    ...
    if (element.settings.isTextFitEnabled) {
      ...
      var relative_size = compartments.computeRelativeCompartmentArea(
        size,
        area_width,
        area_height,
        min_size,
  ...
  addCompartments: function (comparts_in) {
    ...
    _.each(comparts_in, function (compart_in) {
      ...
      if (
        (compart_in && compart_in.type === "text" && compart_in.value === "") ||
        (compart_in.style && compart_in.style.visible === false)
      ) {
        return;
      }

      var compart = new Compartment(
        compartments,
        compart_in,
        compartments.textsGroup,
      );
  ...
  computeCompartmentsArea: function () {
    ...
    return {
      width: Math.round(1.03 * min_width),
      height: Math.round(1.03 * total_height),
    };
  },
  ...
  computeRelativeCompartmentArea: function (
    size,
    area_width,
    area_height,
    min_size,
    ...
    return {
      width: Math.max(area_width, min_shape_width),
  ...
  removeOne: function (compart_id) {
    ...
    compartments.recomputeCompartmentsPosition();
    if (
      compartments.element.presentation &&
      compartments.element.presentation.parent
    ) {
...
Compartment.prototype = {
  createText: function (compart_in) {
    ...
    if (
      compart_in.underline === true ||
      (compart_in.compartmentType && compart_in.compartmentType.underline)
    ) {
      var red_line = new Konva.Line({
        points: [0, 0, 0, 0],
        stroke: "black",
        strokeWidth: 1,
      });
