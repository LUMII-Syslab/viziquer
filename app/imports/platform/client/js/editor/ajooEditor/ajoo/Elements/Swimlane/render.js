...
Swimlane.prototype.addHandlersToLine = function (lines) {
  ...
  _.each(lines, function (line, i) {
    ...
    line.on("mousedown", function (ev) {
      ...
      if (ev.evt.which === 1)
        swimlane.editor.actions.startAction("EditingSwimlane", {
          line: line,
          swimlane: swimlane,
        });
    });
    ...
    line.on("mouseup", function (ev) {
      ...
      if (ev.evt.which === 3) {
        ...
        new Event(swimlane.editor, "rClickedOnElement", {
          ev: ev,
          element: swimlane,
        });
      }
    ...
    line.on("mouseover", function (ev) {
      if (line.name === "horizontal") set_cursor_style("n-resize");
      else if (line.name === "vertical") set_cursor_style("w-resize");
    });
...
Swimlane.prototype.add_swimlane_row = function (line_index) {
  this.add_swimlane_row_or_column(
    line_index,
    1,
    "HorizontalLines",
    "VerticalLines",
...
Swimlane.prototype.add_swimlane_row_or_column = function (
  line_index,
  index,
  lines_group1,
  lines_group2,
...
Swimlane.prototype.replacing_swimlane = function (
  new_line_positions,
  lines_group_name,
  index,
) {
  ...
  if (lines_group_name === "VerticalLines") {
    data = {
      horizontalLines: new_line_positions,
      verticalLines: swimlane.select_lines_position(lines_group, new_points),
    };
  } else {
    data = {
      horizontalLines: swimlane.select_lines_position(lines_group, new_points),
...
Swimlane.prototype.recompute_compartments = function (
  lines_group_name,
  line_index,
) {
  var swimlane = this;
  ...
  var index_obj = {};
  if (lines_group_name === "VerticalLines") index_obj = { rows: line_index };
  else index_obj = { columns: line_index };
...
Swimlane.prototype.recompute_compartments_by_index = function (line_index) {
  ...
  _.each(swimlane.compartments.compartments, function (compartment) {
    ...
    if (row >= r_index) row = row + 1;

    //increasing column index
    var column = compartment.column;
    if (column >= c_index) column = column + 1;
...
Swimlane.prototype.remove_swimlane_row = function (line_index) {
  this.remove_swimlane_row_or_column(
    line_index,
    1,
    "HorizontalLines",
    "VerticalLines",
...
Swimlane.prototype.remove_swimlane_row_or_column = function (
  line_index,
  index,
  lines_group1,
  lines_group2,
...
Swimlane.prototype.recompute_compartments_after_remove = function (
  lines_group,
  line_index,
) {
  var swimlane = this;
  ...
  var r_index, c_index;
  if (lines_group === "VerticalLines") r_index = line_index;
  else c_index = line_index;
...
Swimlane.prototype.buildSwimlaneObject = function (data) {
  ...
  var compartments = _.map(
    swimlane.compartments.compartments,
    function (compart) {
      return {
        _id: compart._id,
        input: compart.input,
