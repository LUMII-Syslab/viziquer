...
ResizingShape.prototype = {
  startDragging: function (element, resizer_name) {
    ...
    if (element.resizers) element.resizers.remove();
  ...
  recomputeLinkedPorts: function (element) {
    ...
    _.each(ports, function (port) {
      ...
      var deltas = [
        { name: "deltaX1", value: Math.abs(port_size.x) },
        {
          name: "deltaX2",
          value: Math.abs(port_parent_size.width - port_size.x),
        },
        { name: "deltaY1", value: Math.abs(port_size.y) },
        {
          name: "deltaY2",
          value: Math.abs(port_parent_size.height - port_size.y),
        },
      ];
  ...
  computeAdjustedPoint: function (new_x, new_y) {
    return SelectionDragging.prototype.computeAdjustedPoint.call(
      this,
      new_x,
      new_y,
    );
  ...
  recomputeDirectLines: function () {
    SelectionDragging.prototype.recomputeDirectLines.call(this, {
      deltaX: 0,
      deltaY: 0,
    });
  },
