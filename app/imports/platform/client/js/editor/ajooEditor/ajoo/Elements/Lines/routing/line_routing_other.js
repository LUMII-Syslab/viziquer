...
var convertArray = function (a) {
  for (var i = 0; i < a.length; i++) a[i] = convert(a[i]);
};
var reconvertArray = function (a) {
  for (var i = 0; i < a.length; i++) a[i] = reconvert(a[i]);
};
var listPrintString = function (list, text, tabs) {
  var str = tabs + "\t" + text + ": ";
  _.each(list, function (value, ind) {
    str += "; " + ind.toString() + ":" + value.toString();
  });
  return str;
};
...
var getValueOfPair = function (pair, len) {
  return pair[0] * len + pair[1];
};
var getPairOfValue = function (v, len) {
  var mod = v % len;
  return [(v - mod) / len, mod];
};
