...
Link.prototype = {
    ...
    line.hitFunc(function (context) {
      ...
      function compute_width(index, func, direction) {
        if (func(points, index))
          if (direction === "to") return width;
          else return -width;
        else if (direction === "to") return -width;
        else return width;
      }
  ...
  setSelectedStyle: function () {
    ...
    var style = {
      elementStyle: { shadowEnabled: true, shadowColor: "red", shadowBlur: 15 },
    };
  ...
  is_vertical: function (points, index) {
    if (points[index] === points[index + 2]) return true;
    else return false;
  },

  is_horizontal: function (points, index) {
    if (points[index + 1] === points[index + 3]) return true;
    else return false;
  },
  ...
  isSelectedWithBothEnds: function () {
    ...
    if (selected[start_elem_id] && selected[end_elem_id]) return true;
  },
