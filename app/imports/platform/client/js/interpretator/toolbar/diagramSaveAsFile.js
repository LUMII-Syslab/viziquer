...
function CreateFile(list) {
  ...
  if (file_types[file_type]) file_types[file_type]();
  else console.log("Can't convert to the specified file format:", file_type);
}
...
function generate_elements_content(docDefinition, diagram_id) {
  ...
  elements.forEach(function (elem) {
    ...
    var copmartments = Compartments.find(
      { elementId: elem._id },
      { sort: { index: 1 } },
    );
