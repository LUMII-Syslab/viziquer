...
Interpreter.methods({
  UpdateCompartment: function (
    ...
    if (compartStyleId) {
      ...
      if (compart_type) {
        compart_style = Dialog.get_style_by_id(
          compart_type.styles,
          compartStyleId,
        );
      }
    ...
    return Dialog.updateCompartmentValue(
      compart_type,
      elem_id,
      input,
      value,
  ...
  UpdateBigCompartment: function (
    ...
    var compart_type = this;
    var compartment = Compartments.findOne({
      elementId: elem_id,
...
//Dialog methods
const Dialog = {
  updateCompartmentValue: function (
    compart_type,
    elem_id,
    input,
    value,
  ...
  buildCompartmentList: function (compart_type, elem_id, input, value) {
    ...
    // if multifield
    if (
      compart_type.inputType.type === "custom" &&
      compart_type.inputType.templateName === "multiField"
    ) {
      var ct_comparts_indexes = Compartments.find(
        { compartmentTypeId: compart_type._id, elementId: compart.elementId },
        { sort: { index: 1 } },
      ).map(function (c) {
        return c.index;
      });
  ...
  buildCompartmentValue: function (
    compart_type,
    input,
    mapped_value,
    elemStyleId,
  ...
  buildSubCompartmentTree: function (parent, compart_type, compart_tree) {
    ...
    _.each(
      _.sortBy(sub_compart_types, function (sc) {
        return sc.index;
      }),
      function (sub_compart_type, i) {
        ...
        if (sub_sub_compart_types.length === 0) {
          ...
          var value = Dialog.buildCompartmentValue(
            sub_compart_type,
            input_value,
            mapped_value,
          );
          sub_compart_tree[sub_compart_type.name] = {
            value: value,
            input: input_value,
          };
          res.push(value);
        } else {
          var val = Dialog.buildSubCompartmentTree(
            parent,
            sub_compart_type,
            sub_compart_tree,
          );
  ...
  renderDialogFields: function (compart_type, compartment) {
    if (compartment) {
      _.extend(compart_type, {
        field_value: compartment.input,
    ...
    if (compart_type.inputType) {
      ...
      var dynamic_drop_down = Interpreter.getExtensionPointProcedure(
        "dynamicDropDown",
        compart_type,
      );
      var values;
      ...
      compart_type.values = _.map(values, function (value_item) {
    ...
    if (compart_type.cloudFiles) {
      ...
      compart_type.addedFiles = DiagramFiles.find({
        elementId: Session.get("activeElement"),
      }).map(function (diagram_file) {
            ...
            if (extensions[file.extension]) {
              ...
              Utilities.callMeteorMethod("getFileUrl", list, function (url) {
                ...
                if (obj.attr("video")) {
                  ...
                  obj.remove();
                  $('<source video="video" type="video/mp4">')
                    .appendTo(parent)
      ...
      compart_type.filesList = CloudFiles.find({
        _id: { $nin: added_files_ids },
      }).map(function (file) {
        if (file.extension) {
  ...
  checkCompartmentVisibility: function (compart_type, compartment) {
    ...
    var is_visible_extension_point = _.find(
      compart_type.extensionPoints,
      function (extension_point) {
        return extension_point.extensionPoint === extension_point_name;
      },
    );

    if (is_visible_extension_point) {
      is_visible = Interpreter.executeExtensionPoint(
        compart_type,
  ...
  buildCopartmentDefaultValue(list) {
    var compartments = [];
    CompartmentTypes.find(
      { elementTypeId: list.elementTypeId },
      { $sort: { index: 1 } },
    ).forEach(function (compart_type) {
      if (
        ...
        var proc_name = Interpreter.getExtensionPointProcedure(
          "dynamicDefaultValue",
          compart_type,
        );
        if (proc_name && proc_name !== "") {
          var val = Interpreter.execute(proc_name, [
            list.startElement,
            list.endElement,
          ]);
...
function override_element_style(style_obj_in) {
  ...
  var style_obj_out = {};
  var is_overriden1 = override_style_properties(
    style_obj_in.elementStyle,
    style_obj_out,
    "elementStyle",
  );
  var is_overriden2 = override_style_properties(
    style_obj_in.startShapeStyle,
    style_obj_out,
    "startShapeStyle",
  );
  var is_overriden3 = override_style_properties(
    style_obj_in.endShapeStyle,
    style_obj_out,
    "endShapeStyle",
  );
...
function override_compartment_style(style_obj_in) {
  ...
  var style_obj_out = {};
  var is_overriden = override_style_properties(
    style_obj_in.style,
    style_obj_out,
  );
...
function get_prefix(compart_type, value) {
  return get_object_type_property(
    compart_type,
    "prefix",
    "dynamicPrefix",
    value,
...
function get_object_type_property(compart_type, property, proc_name, value) {
  var dynamic_suffix = Interpreter.getExtensionPointProcedure(
    proc_name,
    compart_type,
  );
