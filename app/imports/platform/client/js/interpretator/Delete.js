...
import { DiagramTypes, Elements } from "../../../../db/platform/collections.js";
...
Interpreter.methods({
  ...
  Delete: function (e) {
    ...
    if (res !== false) {
      ...
      var linked_elem_ids = Elements.find({
        $or: [
          { startElement: { $in: selected_elem_ids } },
          { endElement: { $in: selected_elem_ids } },
        ],
      }).map(function (elem) {
        return elem._id;
      });
