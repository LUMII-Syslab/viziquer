...
import { _contextMenu } from "../../../../client/lib/global_variables.js";
...
ContextMenu.prototype = {
  ...
  showZoomChartMenu: function (e, obj) {
    ...
    var delta_x = Math.round(
      net_chart_container.offset().left - parent.offset().left,
    );
    var delta_y = Math.round(
      net_chart_container.offset().top - parent.offset().top,
    );
...
function get_context_menu_list(obj_type, property, extension_point) {
  var prop_name = Interpreter.getExtensionPointProcedure(
    extension_point,
    obj_type,
  );
            ...
            item.subMenu = _.map(item.subMenu, function (menu_item, i) {
              ...
              if (menu_item.subMenu) {
                if (dataShapes.schema.hide_individuals === true) {
                  menu_item.subMenu = menu_item.subMenu.filter(function (m) {
                    return m.procedure !== "AddUriName";
                  });
                }

                menu_item.subMenu = _.map(
                  menu_item.subMenu,
                  function (menu_item_3, i) {
                    menu_item_3.index = i;
                    return menu_item_3;
                  },
                );
...
Template.contextMenuTemplate.events({
  ...
  "mouseover .context-menu-item": function (e) {
    ...
    if (context_menu_item.attr("level") === "2") sub_menu_name = "sub-sub-menu";
    ...
    var child_menu = context_menu_item
      .closest(".context-menu-item-li")
      .find("." + sub_menu_name);
...
export { ContextMenu, context_menu_hide, get_context_menu_list };
