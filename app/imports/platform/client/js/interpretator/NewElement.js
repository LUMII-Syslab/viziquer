...
import { Elements, ElementTypes } from "../../../../db/platform/collections.js";
    ...
    //searching through supertypes
      ...
      for (var j = 0; j < end_elem_super_types.length; j++) {
        ...
        for (var i = 0; i < super_types.length; i++) {
          if (
            super_types[i] === start_elem_type_id &&
            end_type_id === end_elem_type_id
          ) {
            continue;
          }

          var tmp_line_type = get_line_type(
            [elem_type_id],
            super_types[i],
            end_type_id,
          );
