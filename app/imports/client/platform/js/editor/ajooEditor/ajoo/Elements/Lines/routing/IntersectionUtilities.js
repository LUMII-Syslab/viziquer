...
var compute_intersection = function(shape_svg_obj1, shape_svg_obj2) {
    ...
    _.each(shape_svg_obj1, function(shape1) {
        ...
        build_shape_object(shape1, shapes);
        _.each(shape_svg_obj2, function(shape2) {

            build_shape_object(shape2, shapes);

            var intersection = get_intersections(shapes[0], shapes[1]);
            if (intersection && intersection["points"]) {

                _.each(intersection["points"], function(point) {
                    intersections["points"].push(point);
                });
            }
        });
    });
...
function build_shape_object(node, shapes) {

    var shape;
    switch (node.localName) {
        case "circle":  shape = new Circle(node);    break;
        case "ellipse": shape = new Ellipse(node);   break;
        case "line":    shape = new Line(node);      break;
        case "path":    shape = new Path(node);      break;
        case "polygon": shape = new Polygon(node);   break;
        case "rect":    shape = new Rectangle(node); break;
        default:
            // do nothing for now
    }
...
function get_intersections(shape1, shape2) {
    return  Intersection.intersectShapes(shape1, shape2);
}
