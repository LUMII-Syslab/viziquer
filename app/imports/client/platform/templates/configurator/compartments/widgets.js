...
Template.compartmentDialog.events({
  ...
  'change #tab': function (e) {
    ...
    if (compartment_type_id != "noSelection") {
      ...
      var list = {
        id: Session.get("compartmentTargetTypeId"),
...
async function update_input_type(list) {
  list["id"] = Session.get("compartmentTargetTypeId");
  await Utilities.callMeteorMethodAsync("updateInputType", list);
}

async function insert_tab(tab_list) {
  var list = { tab: tab_list, compartmentTypeId: Session.get("compartmentTargetTypeId") };
  await Utilities.callMeteorMethodAsync("insertTabWIthCompartmentType", list);
}
...
async function add_selection_item(list) {
  await Utilities.callMeteorMethodAsync("addSelectionItem", list);
}

async function update_selection_object(list) {
  await Utilities.callMeteorMethodAsync("updateSelectionItem", list);
}
