  ...
  "click #ok-upload-files": function (e) {
    ...
    _.each(fileList, function (myFile) {
      ...
      reader.onload = async function () {
        ...
        await Utilities.callMeteorMethodAsync("importConfiguration", list);
...
Template.addConfiguratorDiagram.events({
  ...
  "hidden.bs.modal #add-configurator-diagram": async function (e) {
    ...
    if (src.attr("OKPressed")) {
      ...
      if (diagram_type) {
        ...
        let dgr_obj = await Utilities.callMeteorMethodAsync("insertDiagramType", list);
...
Template.toolbarForm.helpers({
  ...
  // 	await Utilities.callMeteorMethodAsync("updateTool", list);
  // },
...
Template.toolbarForm.events({
  ...
  'click #ok-toolbar-form': async function (e) {
    var list = {
      toolId: Session.get("toolId"),
      set: { toolbar: $("#toolbar-form-name").val(), }
    };
