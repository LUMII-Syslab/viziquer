...
Template.newTab.events({
  ...
  'click #add-tab': function (e) {
    ...
    if (elem_type) {
      ...
      var dialog_tab = DialogTabs.findOne({
        diagramTypeId: Session.get("targetDiagramType"),
        elementTypeId: { $exists: false }
      },
        { sort: { index: -1 } });
...
async function insert_tab(list) {
  await Utilities.callMeteorMethodAsync("insertTab", list);
}

async function update_tab_name(id, name) {
  ...
  await Utilities.callMeteorMethodAsync("updateTab", list);
}

async function remove_tab(tab_id) {
  var list = { id: tab_id, toolId: Session.get("toolId") };
  await Utilities.callMeteorMethodAsync("removeTab", list);
}
