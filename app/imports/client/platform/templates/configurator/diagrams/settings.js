...
Template.diagramSize.helpers({
  size: function () {
    ...
    if (diagram_type && diagram_type["size"]) {
      return {
        diagramSize: diagram_type["size"]["diagramSize"],
...
async function update_diagram_size(list) {
  list["id"] = Session.get("targetDiagramType");
  list["toolId"] = Session.get("toolId");

  await Utilities.callMeteorMethodAsync("updateDiagramSize", list);
}

async function update_selection_or_new_line_style(e, selection_or_line) {
  var pair = Configurator.getInputFieldValue(e);
  var attr_name = pair["_attr"];
  var attr_value = pair["_value"];
