...
Template.userGroups.events({
  ...
  "click #new-group": async function (e) {
    e.preventDefault();
    ...
    await Utilities.callMeteorMethodAsync("addGroup", list);

    return;
  },

  "blur .edit-group-name": async function (e) {
    e.preventDefault();
    ...
    await Utilities.callMeteorMethodAsync("editGroup", list);

    return;
  },

  "click .delete-group-button": async function (e) {
    e.preventDefault();

    var group_id = $(e.target).closest(".table-row").attr("id");

    var list = { id: group_id, projectId: Session.get("activeProject") };
    await Utilities.callMeteorMethodAsync("removeGroup", list);
...
Template.foundUsers.events({
  ...
  'click .invite': async function (e, templ) {
    e.preventDefault();
    ...
    await Utilities.callMeteorMethodAsync("insertProjectsUsers", list);
...
Template.inviteUserForm.events({
  ...
  "click #invite-user-send-button": async function (e) {
    ...
    await Utilities.callMeteorMethodAsync("enrollUser", list);
  },
...
async function set_role(ev, role) {
...
function build_user_search_query(search_entered) {
  ...
  if (search_items.length == 0)
    ...
    if (search_items.length == 1) {
      query = {
        $or: [{ name: { '$regex': "^" + search_items[0], $options: 'i' } },
