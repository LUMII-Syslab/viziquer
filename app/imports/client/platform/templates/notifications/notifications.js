...
import { Notifications, Users, Projects, ProjectsUsers } from '/imports/db/platform/collections'
...
Template.notificationsT.events({
  ...
  'click #nav-notifications': async function (e, templ) {
    var list = {};

    //the server method is used to update multiple notifications at once
    await Utilities.callMeteorMethodAsync("setNotifcationsSeen", list);
  },
});
...
Template.userNotifications.events({
  ...
  'click .notification-delete': async function (e) {
    e.preventDefault();
    var id = $(e.target).closest(".notification-delete").attr("id");

    var list = { id: id };

    await Utilities.callMeteorMethodAsync("removeNotification", list);
  },
...
async function reject_invitation(e) {
  var id = $(e.target).closest(".notification").attr("id");

  var list = { id: id, update: { $set: { status: "rejected" } } };
  await Utilities.callMeteorMethodAsync("updateNotification", list);
}

//accepts the invitation
async function accept_invitation(e) {
