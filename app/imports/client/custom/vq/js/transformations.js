...
Interpreter.customMethods({
  ...
  VQgetAggregateNames: async function () {

    var act_elem = Session.get("activeElement");
    if (!act_elem) {
      return [];
    }
    var act_comp = await Compartments.findOneAsync({ elementId: act_elem })
    if (!act_comp) {
      return [];
    }

    var elem_type = await ElementTypes.findOneAsync({ name: "Class" });
  ...
  VQsetSubQueryInverseLink: async function () {
    //arrow ->compartments->Inverse Link->extensions->after Update
    //params: (compartType, compartId)
    // console.log("SSSAAASSS");
    var ct = await CompartmentTypes.findOneAsync({ name: "Inverse Link" });
    var ctn = await CompartmentTypes.findOneAsync({ name: "Name", elementTypeId: Session.get("activeElementType") });
    var act_elem = Session.get("activeElement");

    if (act_elem && ct && ctn) {
      var c = await Compartments.findOneAsync({ elementId: act_elem, compartmentTypeId: ct["_id"] });
      var cn = await Compartments.findOneAsync({ elementId: act_elem, compartmentTypeId: ctn["_id"] });
      var c_cal = await Compartments.findOneAsync({ elementId: act_elem, input: { $nin: ["true", "false"] } });
  ...
  VQsetLinkName: async function (params) {
    var c = await Compartments.findOneAsync({ _id: params["compartmentId"] });
    if (c) {
  ...
  VQSetHideDefaultLinkValue: async function (a, b, c) {
  ...
  VQsetGraphPrefix: async function (val) {
    var act_elem = Session.get("activeElement");
    var act_el = await Elements.findOneAsync({ _id: act_elem }); //Check if element ID is valid
    var compart_type = await CompartmentTypes.findOneAsync({ name: "Graph instruction", elementTypeId: act_el["elementTypeId"] });
    var compart = await Compartments.findOneAsync({ compartmentTypeId: compart_type["_id"], elementId: act_elem });
  ...
  setIsVisibleForNamedGraphs: async function () {
  ...
  addOuterQuery: async function () {
    ...
    if (await Elements.findOneAsync({ _id: selected_elem_id })) { //Because in case of deleted element ID is still "activeElement"
      ...
      if (currentElement.isClass() && currentElement.isRoot()) {
        ...
        Create_VQ_Element(async function (cl) {
          cl.setName("[ ]");
          var proj = await Projects.findOneAsync({ _id: Session.get("activeProject") });
