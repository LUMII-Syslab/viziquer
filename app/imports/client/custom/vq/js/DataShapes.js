...
const getSchemaServerUrl = async () => {
  //if (_schemaServerUrl) return _schemaServerUrl; // TODO šī saglabašana nezin kāpēc nestrādāja
  return Meteor.callAsync('getEnvVariable', 'SCHEMA_SERVER_URL');
};
// ***********************************************************************************
...
const dataShapes = {
  ...
  makeDiagr: async function (c_list, p_list, superclassType, remSmall, addIds, disconnBig, compView, schema, info) {
    ...
    _.each(rr.data, function (cl) {
      ...
      rezFull.classes[id] = {
        id: cl.id, super_classes: [], sub_classes: [], used: false, used2: false, hasGen: false, isClasif: isClasif, type: type, prefix: cl.prefix, displayName: cl.full_name, cnt: roundCount(cl.cnt),
    ...
    for (const super_class of super_classes_list) {
      ...
      rezFull.classes[sc] = {
        id: super_class.id0, fullName: '', used: true, used2: true, hasGen: true, type: 'Abstract', cl_list: super_class.cl_list_orig, super_classes: [], sub_classes: [],
        ...
        for (const atr of rezFull.classes[`c_${c}`].in_prop) {
          ...
          if (!assoc.removed) {
            ...
            if (atr_tree[p_name] == undefined)
              atr_tree[p_name] = {
                count: 1, id_list: [atr], p_name: `${assoc.from}_${sc}_${assoc.list[0].p_name}`,
                info: {
                  removed: false, from: assoc.from, to: sc, id2: `${assoc.from}_${sc}`,
      ...
      for (const c of super_class.cl_list) {
        for (const atr of rezFull.classes[`c_${c}`].out_prop) {
          ...
          if (!assoc.removed) {
            ...
            if (atr_tree[p_name] == undefined)
              atr_tree[p_name] = {
                count: 1, id_list: [atr], p_name: `${sc}_${assoc.to}_${assoc.list[0].p_name}`,
                info: {
                  removed: false, from: sc, to: assoc.to, id2: `${sc}_${assoc.to}`,
    ...
    function disconnectProp(prop) {
      ...
      if (rezFull.classes[prop.from].type != 'Abstract' && rezFull.classes[prop.to].type != 'Abstract') {
        ...
        c_from.atr_list.push({
          p_name: pr.p_name, p_id: p_id, cnt: -1, object_cnt: pr.cnt, class_name: `-> ${c_to.displayName}`,
      ...
      if (rezFull.classes[prop.from].type != 'Abstract' && rezFull.classes[prop.to].type == 'Abstract') {
        ...
        for (const to_id of c_to_id_list) {
          ...
          c_from.atr_list.push({
            p_name: pr.p_name, p_id: p_id, cnt: -1, object_cnt: pr.cnt, class_name: `-> ${c_to_sub.displayName}`,
            max_cardinality: pr.max_cardinality, is_domain: pr.is_domain
          });
        }
      }
    ...
    if (!compView) {
      ...
      for (const t of Object.keys(temp2)) {
        if (temp2[t].length > 1) {
          ...
          rezFull.classes[sc] = {
            id: `${temp2[t].join('_')}`, fullName: '', displayName: '', used: true, used2: true, hasGen: true, type: 'Abstract', super_classes: [], sub_classes: [],
            atr_list: [], atr_list2: [], in_prop: [], out_prop: [], all_atr: []
          };
  ...
  makeSuperDiagr: async function (c_list, p_list, par, info = '', calk = false) {
    ...
    const printGroups = par.printGroups ? true : false;
    const printDiffs = par.printDiffs ? true : false;
    ...
    if (compTree) diffS = -1; // Ja ir parametrs savilkt visparinašanas, tad arī abstraktās netiek taisītas	
    ...
    _.each(rr.data, function (cl) {
      ...
      if (addIds) full_name = `${full_name} ID-${cl.id}`;
      rezFull.classes[id] = {
        id: id, id_id: cl.id, c_list_id: [cl.id], super_classes: [], used: false, hasGen: false, type: type,
      ...
      for (const classInfo of cl_list) {
        for (const atr of classInfo.atr_list) {
          ...
          if (atrTree[prop] == undefined) {
            atrTree[prop] = {
              class_list: atr.class_list, cnt: atr.cnt, is_domain: atr.is_domain, is_range: atr.is_range, max_cardinality: atr.max_cardinality,
    ...
    function makeClassGroup(list, type, sum = true) {
      ...
      if (list.length > 1) {
        ...
        const cnt = (sum) ? `(~${roundCount(i)})` : `(${roundCount(list[0].cnt)})`;
        let fullName = `${c_list_full.sort((a, b) => { return b.cnt - a.cnt; })[0].displayName} et al. G${Gnum} ${cnt}`;
        let displayName = `${c_list_full.sort((a, b) => { return b.cnt - a.cnt; })[0].displayName} et al.`;
        if (c_list_full.length == 2) {
          fullName = `${c_list_full[0].displayName} or ${c_list_full[1].displayName} G${Gnum} ${cnt}`;
          displayName = `${c_list_full[0].displayName} or ${c_list_full[1].displayName}`;
        }
        rezFull.classes[g_id] = {
          id: g_id, super_classes: [], used: true, hasGen: false, type: type,
    ...
    // Statistikas savākšana, vēlāk nevajadzēs
    ...
    for (const clId1 of Object.keys(rezFull.classes)) {
      for (const clId2 of Object.keys(rezFull.classes)) {
        ...
        if (classInfo1.displayName < classInfo2.displayName) {
          ...
          //if ( diff[0] > 10 &&  diff[1] > 0) // Informatīva druka
    ...
    if (printDiffs) {
      ...
      for (const clId1 of Object.keys(rezFull.classes)) {
        for (const clId2 of Object.keys(rezFull.classes)) {
          ...
          const classInfo2 = rezFull.classes[clId2];
          const diff = getDifference(classInfo1, classInfo2);
          if (classInfo1.displayName < classInfo2.displayName && diff[0] > 0 && classInfo1.used && classInfo2.used) {
    ...
    if (diffG > 0) {
      ...
      // Izdrukā potencialās klašu grupas un beidz darbu
      if (printGroups) {
        rezFull.type = 'printGroups';
        for (const lId of Object.keys(rezFull.lines)) {
          rezFull.classes[rezFull.lines[lId].from].gr = true;
          rezFull.classes[rezFull.lines[lId].to].gr = true;
        }
        for (const cId of Object.keys(rezFull.classes)) {
          if (rezFull.classes[cId].gr)
            rezFull.classes[cId].used = true;
          else
            rezFull.classes[cId].used = false;
        }
        makeDiagramData();
        printDiagram();
        return;
    ...
    function makeSupClass(cl_list, temp) {
      ...
      rezFull.classes[sc_id] = {
        id: sc_id, used: true, hasGen: true,
      ...
      let fullName = `${c_list_full.sort((a, b) => { return b.cnt - a.cnt; })[0].displayName} et al. S${Snum} (~${roundCount(cnt)})`;
      let displayName = `${c_list_full.sort((a, b) => { return b.cnt - a.cnt; })[0].displayName} et al.`;
      if (c_list_full.length == 2) {
        ...
        for (const atr of classInfo.atr_list) {
          if (atr.type == 'out') {
            ...
            for (const to_id of atr.class_list2) {
              ...
              const p_name = (addIds) ? `${atr.p_name}(ID-${atr.p_id})` : atr.p_name;
              if (!has_cpc) {
    ...
    function makeDiagramData() {
      ...
      function getAtrString(atr_info) {
        let rez = '';
        const p_name = (addIds) ? `${atr_info.p_name}(ID-${atr_info.p_id})` : atr_info.p_name;
      ...
      for (const clId of Object.keys(rezFull.classes)) {
        ...
        if (classInfo.used) {
          ...
          if (classInfo.atr_string.length > 300 && p_list.length > 1000) // TODO Te vajag skaistāk darīt
            classInfo.atr_string = `${classInfo.atr_string.slice(0, 300)}\n...`
      ...
      // TODO Pagaidu risinajums 'treknajiem'
      if (p_list.length > 1000) {
        for (const aa of Object.keys(assoc)) {
          const aInfo = assoc[aa];
          if (aInfo.string.length > 100)
            aInfo.string = `${aInfo.string.slice(0, 100)}\n...`
        }
      }
      rezFull.assoc = assoc;
      console.log(rezFull);
    ...
    async function printDiagr(rezFull) {
      ...
      await Meteor.callAsync("importOntology", { projectId: Session.get("activeProject"), versionId: Session.get("versionId") }, table_representation);

    }
    printDiagram();  // TDA varaints
    await printDiagr(rezFull); // Vietējais variants
