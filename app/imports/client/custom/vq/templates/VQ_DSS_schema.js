...
function getParams() {
  let par = {
    addIds: false, disconnBig: $("#disconnBig").val(), hideSmall: $("#hideSmall").val(), compView: $("#compView").is(":checked"), newDifs: true,
    pw: $("#pw").val(), k: 1, diffG: $("#diffG").val(), diffS: 0, supPar: 1, schema: dataShapes.schema.schema, showIntersect: $("#showIntersect").is(":checked")
  }; // withoutGen:$("#withoutGen").is(":checked"),
  //if ( $("#diffG").val() == 10 ) 
  //	par.supPar = 2;
  if ($("#abstr").is(":checked"))
    par.diffS = 50;
...
function setClassProperties(cId) {
  ...
  for (const atr of atr_list) {
    const aInfo = {
      id: `${atr.type} ${atr.p_name}`, display_name: `${atr.p_name} ${atr.type} 
  ...
  for (const classInfo of cl_list) {
    for (const atr of classInfo[key]) {
      ...
      if (atrTree[prop] == undefined) {
        atrTree[prop] = {
          class_list: atr.class_list, cnt: atr.cnt, cnt2: atr.cnt2, is_domain: atr.is_domain, range_id: atr.range_id, max_cardinality: atr.max_cardinality,
...
function makeClassGroup(list, group_type, sum = true) { // ekv = false) {
  ...
  if (list.length > 1) {
    ...
    rezFull.classes[g_id] = {
      id: g_id, super_classes: [], used: true, hasGen: hasGen, type: class_type, group_type: group_type,
...
async function getBasicClasses() {
  ...
  _.each(rr.data, function (cl) {
    ...
    rezFull.classes[id] = {
      id: id, displayName: cl.full_name, id_id: cl.id, c_list_id: [cl.id], super_classes: [], sub_classes: [],
  ...
  for (const p of p_list) {
    ...
    if (p.max_cardinality == -1)
      p.max_cardinality = '*';
    p_list_full[p_id] = {
      id: p.id, p_name: p_name, c_from: c_from, c_to: c_to, iri: p.iri, c_from_full: c_from_full, c_to_full: c_to_full,
  ...
  function addProperty(pp, c_from, c_to) {
    if (c_from.length > 0 && c_to.length == 0) {
      for (const cl of c_from) {
        ...
        const p_info = {
          p_name: pp.p_name, p_id: pp.id, type: 'data', cnt: Number(cl.cnt), cnt2: Number(cl.cnt), object_cnt: Number(cl.object_cnt),
      ...
      for (const c_1 of c_from) {
        ...
        if (c_1.object_cnt > 0) {
          ...
          const p_info = {
            p_name: pp.p_name, p_id: pp.id, type: 'out', cnt: Number(c_1.cnt), cnt2: Number(c_1.cnt), object_cnt: Number(c_1.object_cnt),
          ...
          const p_info = {
            p_name: pp.p_name, p_id: pp.id, type: 'data', cnt: Number(c_1.cnt), cnt2: Number(c_1.cnt), object_cnt: Number(c_1.object_cnt),
            is_domain: pp.is_domain, range_id: '', max_cardinality: pp.max_cardinality, class_list: [], cnt_full: Number(pp.cnt)
          };
          rezFull.classes[from_id].atr_list.push(p_info);
        }
      ...
      for (const c_2 of c_to) {
        ...
        if (has_cpc && cpc_i.length > 0) // ( !compView && has_cpc && cpc_i.length > 0 )
          cl_list = cpc_i.map(c => c.other_class_id);
        const p_info = {
          p_name: pp.p_name, p_id: pp.id, type: 'in', cnt: Number(c_2.cnt), cnt2: Number(c_2.cnt), object_cnt: Number(c_2.object_cnt),
  ...
  for (const aa of Object.keys(rezFull.assoc)) {
    ...
    if (!aInfo.removed) {
      ...
      if (assoc[aID] != undefined) {
        ...
        assoc[aID] = {
          from: aInfo.from, to: aInfo.to, removed: false, string: aInfo.string,
