...
Meteor.methods({
  ...
  executeSparql: function (list) {
    ...
    let sparql_log_entry = Object.assign({}, 
      list,
      { 
        user: user_id, 
    ...
    if (!options.paging_info) {
      ...
      try {
        ...
        let httpOptions = Object.assign({}, authOptions);
          ...
          if (resp["statusCode"] !== 200) {
            ...
            if (resp["headers"]["content-type"].toLowerCase().startsWith('application/json')) {
              try {
                ...
                let json_res = { sparql: JSON.parse(resp.content) };
                if (limit_set) {
                  if (options.paging_info) {
                    json_res.limit = 50;
...
const DO_CALL_DEBUG = function (method, url, options, cb) {
...
const DO_CALL_PROD = function (method, url, options, cb) {
  if (cb) {
    try {
...
function doHttpRequestP1(url, httpOptions, query, namedGraph, preferJSON, callback) {
  ...
  let fullOptions = Object.assign({}, httpOptions, { timeout: TIMEOUT });
  fullOptions.headers = Object.assign({}, COMMON_HEADERS);
  if (preferJSON) {
...
function doHttpRequestP2(url, httpOptions, query, namedGraph, preferJSON, callback) {
  ...
  let fullUrl = url;
  let fullOptions = Object.assign({}, httpOptions, { timeout: TIMEOUT });
  fullOptions.headers = Object.assign(
    {},
    COMMON_HEADERS,
