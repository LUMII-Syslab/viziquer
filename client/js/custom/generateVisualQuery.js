		...
		if(typeof order[key]["expression"] === "string"){
			...
			if(orderTemp["viziQuerExpr"]["exprString"] != ""){
				...
				if(typeof order[key]["expression"]["expression"] == "object"){
					...
					var aggregateExpressionRes = vq_visual_grammar.parse(order[key]["expression"]["expression"])
		...
		if(typeof variables[key] === 'string'){
			...
			if(expression["type"] == "aggregate"){
				...
				if(typeof expression["expression"] == "object"){
					...
					var aggregateExpressionRes = vq_visual_grammar.parse(expression["expression"]);
					aggregateExpression = aggregateExpressionRes["value"];
...
async function parseSPARQLjsStructureWhere(where, nodeList, parentNodeList, classesTable, filterTable, attributeTable, linkTable, selectVariables, bgptype, allClasses, variableList, patternType, bindTable, generateOnlyExpression){
	...
	if(where["type"] == "bind"){
		...
		} else if(typeof where["expression"] !== "string"){
		...
		if(typeof where["expression"] === "string"){
			var bindExprParse = vq_visual_grammar.parse(where["expression"]);
	...
	if(where["type"] == "operation"){
		...
		//coalesce / concat / substr
			...
			if(where["args"].length == 2 && typeof where["args"][1] == 'string'){
				var arg1;
				var arg2 = generateArgument(where["args"][1]);
				if(arg2["type"] == "string"){
					arg2 = arg2["value"];
...
function generateArgument(argument){
	if(typeof argument == 'string'){
		var result = vq_visual_grammar.parse(argument);
