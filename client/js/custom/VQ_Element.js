...
VQ_Schema.prototype = {
  ...
  makeAttributesAndAssociations: function(data) {
	function makeAssoc(asoc, dual) {
		...
		if (schema.type == "New" && extensionMode == "simple" ) // && ( dual == false || dual == true && _.size(asoc.ClassPairs) > 0)) 
		{
			if (asoc.SourceClasses && _.size(asoc.SourceClasses ) > 0)
			{
				_.each(asoc.SourceClasses, function(sc){
					if ( sc.closedRange === false || sc.closedRange === undefined && asoc.closedRange !== true) {
						new_cp = _.union(new_cp,{SourceClass:sc.classFullName, TargetClass:"", instanceCount:sc.objectTripleCount, tripleCount:sc.objectTripleCount,
								sourceImportanceIndex:sc.importanceIndex, targetImportanceIndex:1000}) // Visi tiek salikti kā pēdējie
					}
				})
			}
			...
			if (asoc.TargetClasses && _.size(asoc.TargetClasses) > 0)
			{
				_.each(asoc.TargetClasses, function(sc){
					var inst_count = sc.objectTripleCount;
					if ( sc.closedDomain === false || sc.closedDomain === undefined && asoc.closedDomain !== true)
						new_cp = _.union(new_cp,{SourceClass:"",TargetClass:sc.classFullName,instanceCount:sc.objectTripleCount, tripleCount:sc.objectTripleCount, 
						targetImportanceIndex:sc.importanceIndex, sourceImportanceIndex:1000 }) // Visi tiek salikti kā pēdējie
				})
			}
			//console.log("*********************", asoc.localName ,"*********************************")
			...
			_.each(asoc.TargetClasses, function(sc){
				...
				_.each(new_cp, function(n){
					if ( n.TargetClass == sc.classFullName) {
						...
						n.targetTripleCount = sc.tripleCount;
					}
				})
			})
		}
		...
		_.each(new_cp, function(cp){
			...
			var tClassName = "";
			if (cp.SourceClass != "")
				scClassName = scClass.getElementName();
			if (cp.TargetClass != "")
				tClassName = tClass.getElementName();
			var newSchRole = new VQ_SchemaRole(asoc, cp, newRole);
			newSchRole.sourceClassName = scClassName;
			newSchRole.targetClassName = tClassName;
			newSchRole.dual = cp.dual;
			...
			if (cp.targetImportanceIndex > 0)
				createLink(scClass, newSchRole, "outAssoc", "sourceClass"); 
			else
				newSchRole["sourceClass"] = scClass;
			if (cp.sourceImportanceIndex > 0)
				createLink(tClass, newSchRole, "inAssoc", "targetClass");
			else
				newSchRole["targetClass"] = tClass;
	...
	_.each(data.Attributes, function(atr){
		...
		//console.log("-------------------------")
		//console.log(atr.fullName)
		//console.log(atr.tripleCount + " " +  atr.objectTripleCount) 		
		if ( schema.type == "New" ) {
			_.each(atr.SourceClasses, function (sc){
				if ( sc.tripleCount > sc.objectTripleCount ) {
			...
			_.each(uniqueSourceClasses, function (sc){
				...
				newSchAttr.importanceIndex = 1;
				newSchAttr.tripleCount = -1;
				newSchAttr.dataTripleCount = -1;
				schema.addSchemaAttribute(newSchAttr);
				schema.addSchemaProperty(newSchAttr);
				scClass.addProperty(newSchAttr);
				createLink(newAttr, newSchAttr, "schemaAttribute", "attribute");
  ...
  getSHACL: function(par) {
	...
	var sh_ns = this.namespace;
	if (sh_ns.indexOf("#") != -1) {
		sh_ns = sh_ns.replace("#", "/");
		pref_list = _.union( pref_list, newLine.concat("@prefix shapes: <", sh_ns, "_shapes#> .")); 
	}
	else 
		pref_list = _.union( pref_list, newLine.concat("@prefix shapes: <", sh_ns, "_shapes/> .")); 
	
	if ( par ==  "Full")	
		pref_list = _.union( pref_list, "@prefix shx: <http://viziquer.lumii.lv/shacl_ext/> .");
	
	pref_list = _.union( pref_list, "# This shape definition is auto-generated by the ViziQuer tool (http://viziquer.lumii.lv, https://github.com/LUMII-Syslab/viziquer)"); 
	pref_list = _.union( pref_list, "# after reading the data schema generated by the OBIS Schema Extractor (https://github.com/LUMII-Syslab/OBIS-SchemaExtractor)"); 
	if	(_.find(this.Data.Parameters, function(p){ return  p.name == "endpointUrl" }) !== undefined)
		pref_list = _.union( pref_list, newLine.concat("# from ", _.find(this.Data.Parameters, function(p){ return  p.name == "endpointUrl" }).value,
													" (default graph name ", _.find(this.Data.Parameters, function(p){ return  p.name == "graphName" }).value,").")); 
	var cl_list = [];  
	
	_.each(schema.Classes, function(c){
		if (!c.isAbstract )
		{
			var c_name = c.localName;
			var tc = c.getElementName()
			if (c.isUnique == false && c.ontology.isDefault == false) 
				c_name = newLine.concat(c_name, "_", c.ontology.prefix);

			var sh_cl = [];
			
			sh_cl = _.union(sh_cl,[newLine.concat("shapes:",c_name, "_Shape")]);
			sh_cl = _.union(sh_cl,["\ta sh:NodeShape ;"]);
			sh_cl = _.union(sh_cl,newLine.concat("\tsh:targetClass ", tc, " ;"));
			if ( par ==  "Full")	
				sh_cl = _.union(sh_cl,newLine.concat("\tshx:instanceCount ", c.instanceCount, " ;"));
				
			var attr_list = [];
			function get_end_types(attr) {
				var a11 = "";
				var a12 = "";
				var a21 = "";
				var a22 = "";
				...
				_.each(attr.DataTypes, function(tt){  
					...
					a22 = a22.concat("[sh:datatype ",tt.dataType,"] ");
					cc = cc + 1 })
				if (tripleCountSum < attr.dataTripleCount ) {
					a11 = "sh:nodeKind sh:Literal ";
					a12 = "[sh:nodeKind sh:Literal] ";
					cc = cc + 1
				}
				
				if ( cc == 1 )
					return newLine.concat("\t\t",a21,a11," ;");
				else if ( cc > 1 && a12 == "")
					return newLine.concat("\t\tsh:or ( ",a22,") ;");
				else if ( cc > 1 && a12 != "")
					return newLine.concat("\t\tsh:or ( ",a22,"\n\t\t\t\t",a12,") ;");
				else
					return "";
			...
			function get_end_types_sk(attr) {
				...
				if (_.size(attr.DataTypes) > 0 )
					return newLine.concat("\t\tshx:count_by_datatype ( ",info,") ;");
				else
					return "";
			}
			...
			function make_property(v1, v2, k1, k2, sk, sk2){
				var l = [];	
				...
				if ( par ==  "Full") 
					l = _.union(l,newLine.concat("\t\tshx:tripleCount ", sk, " ;"));
				l = _.union(l,v2); 
				if ( par == "Full")
					l = _.union(l,sk2); 
				l = _.union(l,["\t\]"]);
			...
			function get_end_classes_and_types_sk_out(assoc, link ) {  
				...
				if ( link.sourceDataTripleCount > 0) {
					...
					_.each(link.DataTypes, function(tt){  
						info = info.concat("\n\t\t\t[sh:datatype ",tt.dataType," ; shx:tripleCount ", tt.tripleCount,"]"); })
					if ( info != "" )
						infoFull = infoFull.concat("\n\t\tshx:count_by_datatype ( ",info,") ;");
				}
				return infoFull;
			}
			function get_end_classes_and_types_sk_in(assoc, link ) {  
				var infoFull = "";
			...
			function get_end_classes(assoc, link, class_poz) { 
				...
				var cc = 0;
				if (_.size(tuksais) > 0 ) {
					v11 = "sh:nodeKind sh:IRI";
					v12 = "[sh:nodeKind sh:IRI] ";
					cc = cc + 1;
				}
				if (_.size(link_list) == 1 ) {
					v21 = newLine.concat("sh:class ", link[class_poz].getElementName());
					v22 = newLine.concat("[sh:class ", link[class_poz].getElementName(),"] ");
					cc = cc + 1;
				}
				else if ( _.size(link_list)  > 1 ) {
					_.each(link_list, function(o){ v22 = v22.concat("[sh:class ",o[class_poz].getElementName(),"] "); cc = cc + 1;  }) 
				}
				
				if ( cc == 1 )
					return newLine.concat("\t\t",v21,v11," ;");
				else if ( cc > 1 && v12 == "")
					return newLine.concat("\t\tsh:or ( ",v22,") ;");
				else if ( cc > 1)
					return newLine.concat("\t\tsh:or ( ",v22,"\n\t\t\t\t",v12,") ;");				
				else
					return "";
			...
			function get_end_classes_and_types(assoc, link, class_poz) {  
				var attr = _.find(c.schemaAttribute, function(o) {return o.fullName == link.fullName;}); 
				var a1 = "";
				var a2 = "";
				var tripleCountSum = 0;
				if ( _.size(attr) > 0 ) {
				...
				if (_.size(tuksais) > 0 )
					v1 = "[sh:nodeKind sh:IRI] ";

				_.each(link_list, function(o){ 
					v2 = v2.concat("[sh:class ",o[class_poz].getElementName(),"] ");  }) 
					
				if ( a1 === "" && a2 === "") a1 = "[sh:nodeKind sh:Literal] ";
				if ( v1 === "" && v2 === "") v1 = "[sh:nodeKind sh:IRI] ";
				if ( a1 !== "" && v1 !== "") { 
					if ( a2 === "" && v2 === "" ) s = "sh:nodeKind sh:IriOrLiteral";
					else s = "[sh:nodeKind sh:IriOrLiteral] ";
				}
				else if (a1 !== "") s = a1;
				else if (v1 !== "" ) s = v1;
				
				if ( a2 === "" && v2 === "" )
					return newLine.concat("\t\t",s," ;");
				else if ( a2 === "" )
					return newLine.concat("\t\tsh:or ( ",v2,"\n\t\t\t\t",s,") ;");
				else if ( v2 === "" )
					return newLine.concat("\t\tsh:or ( ",a2,"\n\t\t\t\t",s,") ;");
				else if ( s === "" )
					return newLine.concat("\t\tsh:or ( ",a2,"\n\t\t\t\t",v2,") ;");
				else
					return newLine.concat("\t\tsh:or ( ",a2,"\n\t\t\t\t",v2,"\n\t\t\t\t",s,") ;");
			...
			function transfom_assoc(assoc, v1_pref, v1_suf, class_poz, type){ 
				...
				_.each(assoc, function(link){ 
					if (!link_names[link.fullName] ) // && ( type == "in" || (type == "out" && !link.role.dual ) ))
					{
						...
						var l = [];	
						var v1 = newLine.concat(v1_pref, link.getElementName(), v1_suf); //aaa
						...
						else if (type == "out" && link.dual == true) {
							v2 = get_end_classes_and_types(assoc, link, class_poz);
							sk2 = get_end_classes_and_types_sk_out(assoc, link );
						}
						else if (type == "in") {
							v2 = get_end_classes(assoc, link, class_poz);
							sk2 = get_end_classes_and_types_sk_in(assoc, link );
						}

						if ( v2 != "") {
							l = make_property(v1, v2, k1, k2, link.role.tripleCount, sk2);
			...
			attr_list = _.union(attr_list,transfom_assoc(c.inAssoc, "\t\tsh:path [sh:inverse ", "] ;", "sourceClass", "in"));
	...
	var file_name = "";
	if ( par == "Full")
		file_name = _.find(schema.Ontologies, function(o) {return o.isDefault;}).dprefix.concat("_SHACL_Extended.txt");
	else
		file_name = _.find(schema.Ontologies, function(o) {return o.isDefault;}).dprefix.concat("_SHACL_Basic.txt")
...
VQ_Class.prototype.getAttributes = function() {  
	return _.map(this.schemaAttribute, function (a) {
		...
		maxCard: a.maxCardinality, importanceIndex: a.importanceIndex, dataTripleCount:a.dataTripleCount}; });
  };
...
VQ_Attribute = function (attrInfo){
	...
	if ( _.size(attrInfo.DataTypes) == 0 && type !== undefined)   
		...
		this.sh_type = "\t\tsh:nodeKind sh:Literal";
	else if (tripleCountSum == attrInfo.tripleCount - attrInfo.objectTripleCount && _.size(attrInfo.DataTypes) == 1)   
		this.sh_type = t.concat("\t\tsh:datatype ",type);
	else if (tripleCountSum == attrInfo.tripleCount - attrInfo.objectTripleCount && _.size(attrInfo.DataTypes) > 1)
		this.sh_type = t.concat("\t\tsh:or ( ", sh_type, ") ;");
	else if (sh_type != "")
		this.sh_type = t.concat("\t\tsh:or ( ", sh_type, "[sh:nodeKind sh:Literal]) ;");
	else
		this.sh_type = "\t\tsh:nodeKind sh:Literal";
